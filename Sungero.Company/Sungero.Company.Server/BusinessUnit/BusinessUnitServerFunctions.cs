...
namespace Sungero.Company.Server
{
  partial class BusinessUnitFunctions
  {
    ...
    /// <summary>
    /// Получить наши организации с незаполненным кодом.
    /// </summary>
    /// <returns>Наши организации с незаполненным кодом.</returns>
    [Remote(IsPure = true), Public]
    public static IQueryable<IBusinessUnit> GetBusinessUnitsWithNullCode()
    {
      return BusinessUnits.GetAll().Where(b => b.Status == Status.Active && b.Code == null);
    }
