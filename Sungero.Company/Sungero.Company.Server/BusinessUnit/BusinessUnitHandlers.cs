  ...
  partial class BusinessUnitServerHandlers
  {
    ...
    public override void AfterSave(Sungero.Domain.AfterSaveEventArgs e)
    {
          ...
          e.Params.Contains(Sungero.Commons.PublicConstants.Module.IsIndexedEntityInsertedParamKey))
      {
        ...
        Sungero.Commons.PublicFunctions.Module.CreateIndexEntityAsyncHandler(BusinessUnits.Info.Name,
                                                                             _obj.Id,
                                                                             Functions.BusinessUnit.GetIndexingJson(_obj),
                                                                             allowCreateRecord);
      }
    }
    ...
    public override void Saving(Sungero.Domain.SavingEventArgs e)
    {
      ...
      company.Reliability = Parties.CompanyBase.Reliability.High;
    ...
    {
      #region Удаление замещений руководителя НОР.
      ...
      var managers = Departments.GetAll()
        ...
        .Where(d => Equals(d.BusinessUnit, _obj) && d.HeadOffice == null)
        .Select(d => d.Manager)
        .Where(m => m != null)
        .ToList();
      
      if (managers.Any())
        Sungero.Company.Functions.Module.DeleteUnnecessarySystemSubstitutionsTransaction(managers, _obj.CEO);
    ...
    {
      #region Создание\изменение замещений руководителя НОР.
      ...
      // Изменилось значение свойства руководитель.
      var ceoIsChanged = _obj.State.Properties.CEO.IsChanged &&
        !Equals(_obj.State.Properties.CEO.OriginalValue, _obj.CEO);
      // У НОР ранее был руководитель.
      var businessUnitHadCEO = _obj.State.Properties.CEO.OriginalValue != null;
      // НОР действующая.
      var businessUnitIsActive = _obj.Status == Sungero.CoreEntities.DatabookEntry.Status.Active;
      // НОР стала действующей.
      var businessUnitBecameActivated = _obj.State.Properties.Status.IsChanged &&
        _obj.Status == Sungero.CoreEntities.DatabookEntry.Status.Active;
      // НОР стала закрытой.
      var businessUnitBecameClosed = _obj.State.Properties.Status.IsChanged &&
        _obj.Status == Sungero.CoreEntities.DatabookEntry.Status.Closed;
      
      if (ceoIsChanged && businessUnitHadCEO && (businessUnitIsActive || businessUnitBecameClosed))
        Functions.Module.DeleteUnnecessarySystemSubstitutionsTransaction(managers, _obj.State.Properties.CEO.OriginalValue);
      
      if (_obj.CEO != null)
      {
        if ((businessUnitIsActive && ceoIsChanged) || businessUnitBecameActivated)
          Functions.Module.CreateSystemSubstitutions(managers, _obj.CEO);
        
        if (businessUnitBecameClosed && !ceoIsChanged)
          Functions.Module.DeleteUnnecessarySystemSubstitutionsTransaction(managers, _obj.CEO);
      }
