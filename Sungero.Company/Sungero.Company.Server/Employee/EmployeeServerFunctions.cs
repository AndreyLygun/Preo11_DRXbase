...
namespace Sungero.Company.Server
{
  partial class EmployeeFunctions
  {
    ...
    public static List<IEmployee> GetEmployeesByINILA(string inila)
    {
      ...
      {
        var clearedEmployeeInila = Sungero.Parties.PublicFunctions.Person.RemoveInilaSpecialSymbols(employee.Person.INILA);
    ...
    [Obsolete("Метод не используется с 30.08.2023 и версии 4.8. Используйте метод Parties.PublicFunctions.Person.RemoveInilaSpecialSymbols(string).")]
    public static string RemoveInilaSpecialSymbols(string inila)
    {
      return Sungero.Parties.PublicFunctions.Person.RemoveInilaSpecialSymbols(inila);
    }
    ...
    /// <summary>
    /// Проверить, что сотрудник может готовить проект резолюции для руководителя.
    /// </summary>
    ...
    /// которая внутри себя делает remote-запрос.</remarks>
    [Public]
    public virtual bool CanPrepareDraftResolutionForManager(IEmployee manager)
    {
      var assistants = Functions.Module.GetResolutionPreparers().Where(x => Equals(x.Manager, manager));
      var substitutes = CoreEntities.Recipients.DirectSubstitutionRecipientIdsFor(_obj);
      return assistants.Any(x => substitutes.Contains(x.Assistant.Id) || Equals(x.Assistant, _obj));
    }
    ...
    public virtual bool GetDisableMailNotificationParam()
    {
      ...
      return Sungero.Docflow.PublicFunctions.Module.Remote.GetDocflowParamsBooleanValue(key);
    }
    ...
    {
      var businesUnitIds = new List<long>();
      if (businesUnitId > 0)
    ...
    /// </summary>
    /// <param name="name">Имя.</param>
    /// <param name="businessUnitIds">Список ИД.</param>
    /// <returns>Список сотрудников.</returns>
    [Public]
    public static List<Company.IEmployee> GetEmployeesByNameFuzzy(string name, List<long> businessUnitIds)
    {
      ...
      if (businessUnitIds.Count == 1)
        filter = string.Format("{0},{1}", filter, Commons.PublicFunctions.Module.GetTermQuery("BusinessUnitId", businessUnitIds.Single().ToString()));
      
      if (businessUnitIds.Count > 1)
        filter = string.Format("{0},{1}", filter,
                               Commons.PublicFunctions.Module.GetTermsQuery("BusinessUnitId", businessUnitIds.Select(x => x.ToString()).ToList()));
      
      var employeesIds = new List<long>();
    ...
    [ExpressionElement("GetAssistantWhoPreparesDraftResolutionExpressionName", "GetAssistantWhoPreparesDraftResolutionExpressionDescription")]
    [Public]
    public static List<IRecipient> GetAssistantWhoPreparesDraftResolution(IEmployee employee)
    {
      ...
      var assistant = Company.PublicFunctions.Employee.GetManagerAssistantsWhoPrepareDraftResolution(employee).FirstOrDefault();
      if (assistant != null)
    ...
    [ExpressionElement("GetAssistantWhoPerformsMainAssistantFunctionsExpressionName", "GetAssistantWhoPerformsMainAssistantFunctionsExpressionDescription")]
    [Public]
    public static IRecipient GetAssistantWhoPerformsMainAssistantFunctions(IEmployee employee)
    {
      var assistant = Company.PublicFunctions.Employee.GetManagerAssistantsWhoPerformAssistantMainFunctions(employee).FirstOrDefault();
      if (assistant != null)
    ...
    [ExpressionElement("GetAssistantsExpressionName", "GetAssistantsExpressionDescription")]
    [Public]
    public static List<IEmployee> GetAssistants(IEmployee employee)
    {
      return Functions.Module.GetActiveManagersAssistants()
        .Where(m => Equals(employee, m.Manager))
    ...
    {
      return Functions.Employee.GetShortName(employee, false);
    }
  }
}