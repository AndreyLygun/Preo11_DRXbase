...
namespace Sungero.Company.Shared
{
  partial class EmployeeFunctions
  {
    ...
    /// <summary>
    /// Получить помощников руководителя, выполняющих основные функции помощника руководителя.
    /// </summary>
    /// <returns>Список помощников, выполняющих основные функции помощника руководителя.</returns>
    [Public]
    public virtual List<IManagersAssistant> GetManagerAssistantsWhoPerformAssistantMainFunctions()
    {
      return this.GetManagerAssistants()
        .Where(x => x.IsAssistant == true)
        .ToList();
    }
