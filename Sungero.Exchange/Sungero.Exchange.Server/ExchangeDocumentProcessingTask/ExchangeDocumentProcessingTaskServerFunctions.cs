...
namespace Sungero.Exchange.Server
{
  partial class ExchangeDocumentProcessingTaskFunctions
  {
    ...
    /// <summary>
    /// Сгенерировать тему задания. 
    /// </summary>
    /// <param name="task">Задача задания.</param>
    /// <returns>Тема задания.</returns>
    [ExpressionElement("ExchangeDocumentProcessingTaskDefaultSubject", "")]
    public static string GetExchangeDocumentProcessingTaskSubject(IExchangeDocumentProcessingTask task)
    {
      var dateWithUTC = Sungero.Docflow.PublicFunctions.Module.GetDateWithUTCLabel(task.IncomeDate.Value);
      var businessUnitBox = ExchangeCore.PublicFunctions.BoxBase.GetRootBox(task.Box); 
      return task.IsIncoming == true ?
        ExchangeDocumentProcessingTasks.Resources.AssignmentSubjectFormat(task.Counterparty, businessUnitBox.BusinessUnit, dateWithUTC, task.ExchangeService) :
        ExchangeDocumentProcessingTasks.Resources.AssignmentSubjectFormat(businessUnitBox.BusinessUnit, task.Counterparty, dateWithUTC, task.ExchangeService);
    }
  }
}