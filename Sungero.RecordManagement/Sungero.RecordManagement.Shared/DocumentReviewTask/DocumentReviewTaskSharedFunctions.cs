...
namespace Sungero.RecordManagement.Shared
{
  partial class DocumentReviewTaskFunctions
  {
    ...
    public virtual void FillDraftResolutionProperties(Sungero.Docflow.IOfficialDocument primaryDocument,
                                                      ...
                                                      IActionItemExecutionTask actionItem)
    {
      ...
      Functions.ActionItemExecutionTask.SynchronizeDocumentGroupsToActionItem(actionItem, primaryDocument, addenda, otherAttachments);
      // Признак проставляется после синхронизации чтобы не срабатывала валидация в событии создания вложений в группе "Проект резолюции".
      actionItem.IsDraftResolution = true;
    }
    
    /// <summary>
    /// Синхронизировать вложения из задачи на рассмотрение в проекты резолюции.
    /// </summary>
    public virtual void SynchronizeDocumentReviewAttachmentsToDraftResolutions()
    {
      ...
      {
        Functions.ActionItemExecutionTask.SynchronizeDocumentGroupsToActionItem(draftResolution, primaryDocument, addendaDocuments, otherAttachments);
      }
    }
    ...
    /// <summary>
    /// Синхронизировать группы документов в задачу на рассмотрение.
    /// </summary>
    /// <param name="primaryDocument">Основной документ.</param>
    /// <param name="addenda">Вложения из группы "Приложения".</param>
    /// <param name="otherAttachments">Вложения из группы "Дополнительно".</param>
    [Public]
    public virtual void SynchronizeDocumentGroupsToDocumentReviewTask(Sungero.Docflow.IOfficialDocument primaryDocument,
                                                                      List<Sungero.Content.IElectronicDocument> addenda,
                                                                      List<Sungero.Domain.Shared.IEntity> otherAttachments)
    {
      this.SynchronizePrimaryDocumentToDocumentReviewTask(primaryDocument);
      this.SynchronizeAddendaToDocumentReviewTask(addenda);
      this.SynchronizeOthersGroupToDocumentReviewTask(otherAttachments);
    }
    
    /// <summary>
    /// Синхронизировать основной документ в задачу на рассмотрение.
    /// </summary>
    /// <param name="primaryDocument">Основной документ.</param>
    [Public]
    public virtual void SynchronizePrimaryDocumentToDocumentReviewTask(Sungero.Docflow.IOfficialDocument primaryDocument)
    {
      // При заполнении основного документа будет выполнена синхронизация приложений.
      if (primaryDocument != null && !_obj.DocumentForReviewGroup.OfficialDocuments.Any())
        _obj.DocumentForReviewGroup.OfficialDocuments.Add(primaryDocument);
    }
    
    /// <summary>
    /// Синхронизировать вложения из группы "Приложения" в задачу на рассмотрение.
    /// </summary>
    /// <param name="addenda">Список документов из группы "Приложение".</param>
    [Public]
    public virtual void SynchronizeAddendaToDocumentReviewTask(List<Sungero.Content.IElectronicDocument> addenda)
    {
      if (addenda == null)
        return;
      
      var addendasToRemove = _obj.AddendaGroup.All.Except(addenda);
      foreach (var addendaToRemove in addendasToRemove)
      {
        _obj.AddendaGroup.All.Remove(addendaToRemove);
      }
      
      var addendasToAdd = addenda.Except(_obj.AddendaGroup.All);
      foreach (var addendaToAdd in addendasToAdd)
      {
        _obj.AddendaGroup.All.Add(addendaToAdd);
      }
    }
    
    /// <summary>
    /// Синхронизировать вложения из группы "Дополнительно" в задачу на рассмотрение.
    /// </summary>
    /// <param name="otherAttachments">Список документов из группы "Дополнительно".</param>
    [Public]
    public virtual void SynchronizeOthersGroupToDocumentReviewTask(List<Sungero.Domain.Shared.IEntity> otherAttachments)
    {
      if (otherAttachments == null)
        return;
      
      var othersToRemove = _obj.OtherGroup.All.Except(otherAttachments);
      foreach (var otherToRemove in othersToRemove)
      {
        _obj.OtherGroup.All.Remove(otherToRemove);
      }
      
      var othersToAdd = otherAttachments.Except(_obj.OtherGroup.All);
      foreach (var otherToAdd in othersToAdd)
      {
        _obj.OtherGroup.All.Add(otherToAdd);
      }
    }
    ...
    public virtual void SynchronizeAddendaAndAttachmentsGroup()
    {
      ...
      var addendaToAdd = documentAddenda
        .Except(taskAddenda)
        .Where(x => !taskRemovedAddenda.Contains(x.Id))
    ...
    [Obsolete("Метод не используется с 26.04.2024 и версии 4.10. Теперь вложения из задачи в подзадачу синхронизируются платформой. Приложения синхронизируются в событии Заполнение вложений группы вложений Приложения.")]
