  ...
  partial class ActionItemExecutionTaskFunctions
  {
    ...
    public static string FormatActionItemForSubject(string actionItem, bool hasDocument)
    {
      ...
      if (formattedActionItem.Length > 50 && hasDocument)
        formattedActionItem = formattedActionItem.Substring(0, 50) + Sungero.RecordManagement.ActionItemExecutionTasks.Resources.Ellipsis_FormattedActionItem;
    ...
    /// <summary>
    /// Проверить, разрешено ли редактировать вложения в проекте резолюции.
    /// </summary>
    /// <returns>True, если редактировать вложения разрешено.</returns>
    public virtual bool IsDraftResolutionEditAttachmentsAllowed()
    {
      bool workingWithGUI = Sungero.Commons.PublicFunctions.Module.EntityParamsContainsKey(_obj, Constants.ActionItemExecutionTask.WorkingWithGUI);
      if (_obj.IsDraftResolution == true && _obj.Status == Workflow.Task.Status.Draft && workingWithGUI)
        return false;
    ...
    /// <summary>
    /// Синхронизировать группы документов в поручение.
    /// </summary>
    ...
    [Public]
    public virtual void SynchronizeDocumentGroupsToActionItem(Sungero.Docflow.IOfficialDocument primaryDocument,
                                                              List<Sungero.Content.IElectronicDocument> addenda,
                                                              List<Sungero.Domain.Shared.IEntity> otherAttachments)
    {
      this.SynchronizePrimaryDocumentToActionItem(primaryDocument);
      this.SynchronizeAddendaToActionItem(addenda);
      this.SynchronizeOthersGroupToActionItem(otherAttachments);
    }
    
    /// <summary>
    /// Синхронизировать основной документ в поручение.
    /// </summary>
    /// <param name="primaryDocument">Основной документ.</param>
    [Public]
    public virtual void SynchronizePrimaryDocumentToActionItem(Sungero.Docflow.IOfficialDocument primaryDocument)
    {
      // При заполнении основного документа будет выполнена синхронизация приложений.
      if (primaryDocument != null && !_obj.DocumentsGroup.OfficialDocuments.Any())
        _obj.DocumentsGroup.OfficialDocuments.Add(primaryDocument);
    }
    
    /// <summary>
    /// Синхронизировать вложения из группы "Приложения" в поручение.
    /// </summary>
    /// <param name="addenda">Список документов из группы "Приложение".</param>
    [Public]
    public virtual void SynchronizeAddendaToActionItem(List<Sungero.Content.IElectronicDocument> addenda)
    {
      if (addenda == null)
        return;
    ...
    /// <summary>
    /// Синхронизировать вложения из группы "Дополнительно" в поручение.
    /// </summary>
    /// <param name="otherAttachments">Список документов из группы "Дополнительно".</param>
    [Public]
    public virtual void SynchronizeOthersGroupToActionItem(List<Sungero.Domain.Shared.IEntity> otherAttachments)
    {
    ...
    /// </summary>
    [Obsolete("Метод не используется с 26.04.2024 и версии 4.10. Добавление и удаление вложений вручную теперь учитывается в платформе.")]
