...
namespace Sungero.RecordManagement.Shared
{
  public class ModuleFunctions
  {
    ...
    /// <remarks>Вложения копируются из задач на согласование по регламенту/процессу, рассмотрение документа и на исполнение поручения.</remarks>
    [Public]
    public virtual void SynchronizeAttachmentsToActionItem(Sungero.Workflow.ITask parentTask, IActionItemExecutionTask actionItem)
    {
      ...
      else if (Sungero.DocflowApproval.DocumentFlowTasks.Is(parentTask))
        this.SynchronizeAttachmentsFromDocumentFlowToActionItem(Sungero.DocflowApproval.DocumentFlowTasks.As(parentTask), actionItem);
    }
    ...
    public virtual void SynchronizeAttachmentsFromDocumentReviewToActionItem(IDocumentReviewTask documentReview, IActionItemExecutionTask actionItem)
    {
      ...
      if (actionItem != null)
        Functions.ActionItemExecutionTask.SynchronizeDocumentGroupsToActionItem(actionItem, primaryDocument, taskAddenda, otherAttachments);
    }
    ...
    public virtual void SynchronizeAttachmentsFromActionItemToActionItem(IActionItemExecutionTask parentActionItem, IActionItemExecutionTask actionItem)
    {
      ...
      var addendaDocuments = parentActionItem.AddendaGroup.OfficialDocuments.Select(x => ElectronicDocuments.As(x)).ToList();
    ...
    /// <summary>
    /// Синхронизировать вложения из задачи на согласование документа по процессу во вновь созданное поручение.
    /// </summary>
    /// <param name="documentFlowTask">Задача на согласование документа по процессу.</param>
    /// <param name="actionItem">Поручение.</param>
    public virtual void SynchronizeAttachmentsFromDocumentFlowToActionItem(IDocumentFlowTask documentFlowTask, IActionItemExecutionTask actionItem)
    {
      var primaryDocument = OfficialDocuments.As(documentFlowTask.DocumentGroup.ElectronicDocuments.FirstOrDefault());
      var taskAddenda = documentFlowTask.AddendaGroup.ElectronicDocuments.ToList();
      var otherAttachments = documentFlowTask.OtherGroup.All.ToList();
    ...
    [Public, Obsolete("Метод не используется с 26.04.2024 и версии 4.10. В новом механизме синхронизации вложений группы 'Приложения' и 'Дополнительно' синхронизируются автоматически, если включена соответствующая галочка в группе.")]
    ...
    public virtual void SynchronizeAttachmentsFromApprovalToDocumentReview(Sungero.Docflow.IApprovalTask approvalTask, IDocumentReviewTask documentReview)
    {
      ...
      if (documentReview != null)
        Functions.DocumentReviewTask.SynchronizeDocumentGroupsToDocumentReviewTask(documentReview, primaryDocument, taskAddenda, otherAttachments);
    }
    ...
    public virtual void SynchronizeAttachmentsFromDocumentReviewToDocumentReview(IDocumentReviewTask parentDocumentReview, IDocumentReviewTask documentReview)
    {
      ...
      var taskAddenda = Sungero.RecordManagement.Functions.DocumentReviewTask.GetAddendaGroupAttachments(parentDocumentReview);
