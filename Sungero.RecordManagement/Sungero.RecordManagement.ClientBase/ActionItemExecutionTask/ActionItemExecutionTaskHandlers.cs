...
namespace Sungero.RecordManagement
{
  ...
  partial class ActionItemExecutionTaskClientHandlers
  {
    ...
    public override void Showing(Sungero.Presentation.FormShowingEventArgs e)
    {
      ...
      // Параметр, добавленный в e.Params, доступен только в обработчиках событий, поэтому добавляем еще и в ExtendedEntity.
      var actionItemParams = ((Sungero.Domain.Shared.IExtendedEntity)_obj).Params;
      if (!actionItemParams.ContainsKey(Constants.ActionItemExecutionTask.WorkingWithGUI))
        actionItemParams.Add(Constants.ActionItemExecutionTask.WorkingWithGUI, true);
      ...
      this.CheckActionItemDraftDeadline(e);
      _obj.State.Controls.Control.Refresh();
    }
    ...
    /// <summary>
    /// Проверить, что срок черновика поручения не превышает срок ведущего поручения.
    /// </summary>
    /// <param name="e">Аргументы.</param>
    private void CheckActionItemDraftDeadline(Sungero.Presentation.FormShowingEventArgs e)
    {
      if (!_obj.Deadline.HasValue || _obj.Status != ActionItemExecutionTask.Status.Draft)
        return;

      var parentAssignment = ActionItemExecutionAssignments.As(_obj.ParentAssignment);
      if (parentAssignment != null)
      {
        if (!parentAssignment.ActionItemDraftGroup.ActionItemExecutionTasks.Contains(_obj))
          return;

        if (!parentAssignment.Deadline.HasValue)
          return;
        
        var parentDeadline = parentAssignment.Deadline.Value;
        if (Docflow.PublicFunctions.Module.CheckDeadline(_obj.Deadline.Value, parentDeadline))
          e.AddWarning(ActionItemExecutionTasks.Resources.DeadlineSubActionItemExecutionFormat(parentDeadline.ToUserTime().ToShortDateString()));
      }
    }
  }
}