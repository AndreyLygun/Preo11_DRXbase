...
namespace Sungero.RecordManagement.Client
{
  partial class AcquaintanceTaskFunctions
  {
    ...
    public virtual bool NeedShowSignRecommendation(bool isElectronicAcquaintance, Docflow.IOfficialDocument document)
    {
      ...
      var hasApprovalSignatures = Signatures
        .Get(document.LastVersion, q => q.Where(s => s.SignatureType == SignatureType.Approval))
        .Any();
      return !hasApprovalSignatures && inWhiteList;
    }
    
    /// <summary>
    /// Получить номер версии документа на ознакомление.
    /// </summary>
    /// <returns>Номер версии.</returns>
    public int GetAcquaintanceVersionNumber()
    {
      return _obj.AcquaintanceVersions.FirstOrDefault(x => x.IsMainDocument.Value).Number.Value;
    }
  }
}