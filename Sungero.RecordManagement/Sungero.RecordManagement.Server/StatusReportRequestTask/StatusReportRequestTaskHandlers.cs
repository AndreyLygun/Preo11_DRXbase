    ...
    public virtual IQueryable<T> AssigneeFiltering(IQueryable<T> query, Sungero.Domain.PropertyFilteringEventArgs e)
    {
      ...
      if (parentTask != null)
        query = parentTask.IsCompoundActionItem ?? false ?
          query.Where(u => Functions.Module.GetUnfinishedActionItemsAssignees(parentTask).Contains(u)) :
          query.Where(u => u.Equals(parentTask.Assignee));
      
      if (parentAssignment != null)
        query = query.Where(u => Functions.Module.GetUnfinishedSubActionItemsAssignees(parentAssignment).Contains(u));
