    ...
    public virtual void CompleteAssignment4(Sungero.RecordManagement.IActionItemExecutionAssignment assignment, Sungero.RecordManagement.Server.ActionItemExecutionAssignmentArguments e)
    {
      ...
      if (mainDocument != null)
      {
        foreach (var document in _obj.ResultGroup.OfficialDocuments.Where(d => !Equals(d, mainDocument)))
        {
          try
          {
            if (!document.Relations.GetRelatedFromDocuments(Constants.Module.SimpleRelationRelationName).Contains(mainDocument))
            {
          ...
          {
            Logger.ErrorFormat("CompleteAssignment4. Relation adding error: {3}. Task ID = {0}, Document ID = {1}, Main document ID = {2}", _obj.Id, document.Id, mainDocument.Id, ex.Message);
            throw;
          }
          finally
          {
