...
namespace Sungero.RecordManagement.Server
{
  public partial class ModuleInitializer
  {
    ...
    {
      Sungero.Commons.PublicInitializationFunctions.Module.ModuleVersionInit(this.FirstInitializing, Constants.Module.Init.RecordManagement.Name, Version.Parse(Constants.Module.Init.RecordManagement.FirstInitVersion));
      Sungero.Commons.PublicInitializationFunctions.Module.ModuleVersionInit(this.Initializing49, Constants.Module.Init.RecordManagement.Name, Version.Parse(Constants.Module.Init.RecordManagement.Version49));
      Sungero.Commons.PublicInitializationFunctions.Module.ModuleVersionInit(this.Initializing411, Constants.Module.Init.RecordManagement.Name, Version.Parse(Constants.Module.Init.RecordManagement.Version411));
    }
    ...
    public virtual void FirstInitializing()
    {
      ...
      // Добавление в таблицу параметров минимального количества документов в обучающей выборке для публикации модели.
      AddPublishModelTrainSetCountParam();
    }
    ...
    public static void CreateRoles()
    {
      ...
      var registrationIncomingDocumentRole = Roles.GetAll().FirstOrDefault(r => r.Sid == Docflow.Constants.Module.RoleGuid.RegistrationIncomingDocument);
      var registrationOutgoingDocumentRole = Roles.GetAll().FirstOrDefault(r => r.Sid == Docflow.Constants.Module.RoleGuid.RegistrationOutgoingDocument);
      var registrationInternalDocumentRole = Roles.GetAll().FirstOrDefault(r => r.Sid == Docflow.Constants.Module.RoleGuid.RegistrationInternalDocument);
      var registrationRoles = new List<IRecipient> { registrationIncomingDocumentRole, registrationOutgoingDocumentRole, registrationInternalDocumentRole };
      Docflow.PublicInitializationFunctions.Module.CreateRole(RecordManagement.Resources.RoleNameUsersWithAccessToRecordManagement,
                                                              RecordManagement.Resources.DescritionUsersWithAccessToRecordManagementRole,
                                                              Constants.Module.UsersWithAccessToRecordManagementRole,
                                                              registrationRoles);
    }
    ...
    public static void GrantRightOnFolders(IRole allUsers)
    {
      ...
      try
      {
        ...
        RecordManagement.SpecialFolders.DocumentsToReturn.AccessRights.Grant(allUsers, DefaultAccessRightsTypes.Read);
        RecordManagement.SpecialFolders.OrdersCompanyDirectives.AccessRights.Grant(allUsers, DefaultAccessRightsTypes.Read);
        RecordManagement.SpecialFolders.DocumentsToReturn.AccessRights.Save();
        RecordManagement.SpecialFolders.OrdersCompanyDirectives.AccessRights.Save();
      }
      finally
      {
    ...
    public static void CreateDocumentKinds()
    {
      ...
      var actions = new Domain.Shared.IActionInfo[] { OfficialDocuments.Info.Actions.SendActionItem,
        OfficialDocuments.Info.Actions.SendForDocumentFlow,
    ...
    /// <summary>
    /// Добавить в таблицу параметров значение по умолчанию минимального количества документов в обучающей выборке для публикации модели.
    /// </summary>
    public static void AddPublishModelTrainSetCountParam()
    {
      if (Docflow.PublicFunctions.Module.GetDocflowParamsValue(Constants.Module.MinTrainingSetSizeToPublishClassifierModelParamName) == null)
        Docflow.PublicFunctions.Module.InsertOrUpdateDocflowParam(Constants.Module.MinTrainingSetSizeToPublishClassifierModelParamName,
                                                                  Constants.Module.MinTrainingSetSizeToPublishClassifierModel.ToString());
    }
  }
}
