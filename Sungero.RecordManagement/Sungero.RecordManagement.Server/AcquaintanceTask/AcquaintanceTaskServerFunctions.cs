...
namespace Sungero.RecordManagement.Server
{
  partial class AcquaintanceTaskFunctions
  {
    ...
    public Sungero.Core.StateView GetStateView()
    {
      ...
      // Учитываются созданные к этому моменту UserActionBlock и TaskBlock.
      var totalBlocksCount = 2;
      ...
      totalBlocksCount += acquaintedAssignments.Count;
      totalBlocksCount += attentionAssignments.Count;
      // Если несколько выполненных заданий, то добавляется группировочный блок, в который добавляются блоки заданий.
      if (acquaintedAssignments.Count > 1)
        totalBlocksCount++;
      if (Sungero.Docflow.PublicFunctions.Module.IsBlocksNumberOverLimit(totalBlocksCount))
      ...
      if (finishAssignments.Any())
      {
        totalBlocksCount += finishAssignments.Count;
        if (Sungero.Docflow.PublicFunctions.Module.IsBlocksNumberOverLimit(totalBlocksCount))
          throw AppliedCodeException.Create(Sungero.Docflow.Resources.ExceededNumberOfBlocks);
      }
    ...
    /// </summary>
    /// <param name="task">Задача на ознакомление.</param>
    ...
    public static bool IsDocumentVersionSignatureValid(Docflow.IOfficialDocument document, int version)
    {
      ...
      var signatures = Signatures.Get(documentVersion, q => q.Where(s => s.SignatureType == SignatureType.Approval));
    ...
    {      
      if (Docflow.PublicFunctions.Module.CheckDeadline(newDeadline, _obj.Deadline))
      {
