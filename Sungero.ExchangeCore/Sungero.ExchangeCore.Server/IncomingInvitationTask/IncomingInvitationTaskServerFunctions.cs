...
namespace Sungero.ExchangeCore.Server
{
  partial class IncomingInvitationTaskFunctions
  {
    ...
    public static IIncomingInvitationTask Create(Parties.ICounterparty counterparty, IBusinessUnitBox box, string organizationId, string comment)
    {
      ...
      if (!Docflow.PublicFunctions.Module.IsTaskUsingOldScheme(invitationTask))
        PublicFunctions.IncomingInvitationTask.FillProcessKind(invitationTask);
      ...
      invitationTask.Subject = Docflow.PublicFunctions.Module.CutText(subject, invitationTask.Info.Properties.Subject.Length);
    ...
    [ExpressionElement("IncomingInvitationAssignmentDefaultSubject", "")]
    public static string GetIncomingInvitationAssignmentSubject(IIncomingInvitationTask task)
    {
      var box = task.Box;
      return IncomingInvitationTasks.Resources.AssignmentSubjectFormat(box.BusinessUnit.Name, task.Counterparty.Name, box.ExchangeService.Name);
    }    
  }
}