...
namespace Sungero.Shell.Server
{
  public class ModuleFunctions
  {
    ...
    public virtual List<long> GetWidgetDepartmentIds(Enumeration performer)
    {
      ...
      var currentRecipients = Docflow.PublicFunctions.Module.GetCurrentRecipients(false);
      ...
      if (Equals(performer, Widgets.TopLoadedPerformersGraph.CarriedObjects.MyDirectDepts) ||
        ...
        departmentsIds = Docflow.PublicFunctions.Module.GetManagersDepartments(currentRecipients);
        ...
        departmentsIds = Docflow.PublicFunctions.Module.GetCEODepartments(currentRecipients);
    ...
    {
      var needPrint = stageType == Docflow.ApprovalReviewAssignmentCollapsedStagesTypesRe.StageType.Print;
      var needRegister = stageType == Docflow.ApprovalReviewAssignmentCollapsedStagesTypesRe.StageType.Register;
      var needCheckSending = stageType == Docflow.ApprovalReviewAssignmentCollapsedStagesTypesRe.StageType.Sending;
      var needCheckPrint = needCheckSending || needPrint;
      var needCheckRegister = needCheckPrint || needRegister;
      ...
      query = query.Where(q => needCheckReview && ApprovalReviewAssignments.Is(q) && ApprovalReviewAssignments.As(q).CollapsedStagesTypesRe.Any(s => s.StageType == stageType) ||
                          ...
                          needPrint && DocumentProcessingAssignments.Is(q) && DocumentProcessingAssignments.As(q).PrintDocument == true ||
                          needRegister && DocumentProcessingAssignments.Is(q) && DocumentProcessingAssignments.As(q).RegisterDocument == true);
    ...
    /// </summary>
    /// <returns>ИД документа, в который сохранен отчет.</returns>
    [Public(WebApiRequestType = RequestType.Get)]
    ...
    {
      var document = SimpleDocuments.Get(documentId);
      if (document == null)
        throw AppliedCodeException.Create(string.Format("Check: Документ с Id {0} не найден", documentId));
      
      var retryCount = 0;
      var maxRetryCount = 20;
      var lockInfo = Locks.GetLockInfo(document);
      while (lockInfo != null && lockInfo.IsLocked && retryCount++ < maxRetryCount)
      {
        System.Threading.Thread.Sleep(1000);
      }
      if (retryCount >= maxRetryCount)
        throw AppliedCodeException.Create(string.Format("Check: Превышено количество попыток ожидания разблокировки документа с Id {0}", documentId));
    ...
    public virtual int CheckModulesVersion(string version)
    {
      ...
      foreach (var moduleInfo in licenseInfo.AppliedModulesInfo)
      {
        var infoMessage = string.Format("Check: версия модуля \"{0}\": {1}, заданная версия: {2}", moduleInfo.ModuleName, moduleInfo.Version, version);
        if (moduleInfo.Version != version)
          throw AppliedCodeException.Create(infoMessage);
        Logger.DebugFormat(infoMessage);
      }
