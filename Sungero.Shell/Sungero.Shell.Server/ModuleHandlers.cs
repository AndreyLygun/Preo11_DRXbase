...
namespace Sungero.Shell.Server
{
  ...
  partial class ApprovalFolderHandlers
  {
    ...
    {
      // Фильтр по типу.
      var typeFilterEnabled = _filter != null && (_filter.RuleBased || _filter.ProcessBased || _filter.Free);
      var showRuleBasedApproval = !typeFilterEnabled || _filter.RuleBased;
      var showProcessBasedApproval = !typeFilterEnabled || _filter.ProcessBased;
      var showFreeApproval = !typeFilterEnabled || _filter.Free;
      var result = query
        .Where(t => showRuleBasedApproval && ApprovalTasks.Is(t) ||
               showProcessBasedApproval && DocumentFlowTasks.Is(t) ||
  ...
  partial class OnApprovalFolderHandlers
  {
    ...
    {
      // Фильтр по типу.
      var typeFilterEnabled = _filter != null && (_filter.RuleBased || _filter.ProcessBased || _filter.Free || _filter.Other);
      var showRuleBasedApproval = !typeFilterEnabled || _filter.RuleBased;
      var showProcessBasedApproval = !typeFilterEnabled || _filter.ProcessBased;
      var showFreeApproval = !typeFilterEnabled || _filter.Free;
      var showOtherApproval = !typeFilterEnabled || _filter.Other;
      var result = query
        .Where(a =>
               // Согласование по регламенту.
               showRuleBasedApproval && (ApprovalAssignments.Is(a) || ApprovalManagerAssignments.Is(a)) ||
               // Согласование по процессу.
               showProcessBasedApproval && EntityApprovalAssignments.Is(a) && DocumentFlowTasks.Is(a.Task) ||
               // Свободное согласование.
               showFreeApproval && (FreeApprovalAssignments.Is(a) || EntityApprovalAssignments.Is(a) && FreeApprovalTasks.Is(a.Task)) ||
               // Прочее.
               showOtherApproval && EntityApprovalAssignments.Is(a) && !DocumentFlowTasks.Is(a.Task) && !FreeApprovalTasks.Is(a.Task));
  ...
  partial class OnDocumentProcessingFolderHandlers
  {
    ...
    public virtual IQueryable<Sungero.Workflow.IAssignmentBase> OnDocumentProcessingDataQuery(IQueryable<Sungero.Workflow.IAssignmentBase> query)
    {
      ...
      var result = query.Where(q =>
                               ...
                               DocumentProcessingAssignments.Is(q) && DocumentProcessingAssignments.As(q).CreateActionItems == true ||
                               ...
                               DocumentProcessingAssignments.Is(q) && DocumentProcessingAssignments.As(q).SendToCounterparty == true ||
                               // Контроль возврата.
                               showCheckReturn && (ApprovalCheckReturnAssignments.Is(q) || DocflowApproval.CheckReturnFromCounterpartyAssignments.Is(q)) ||
                               // Прочие задания.
                               showOther && (ApprovalSimpleAssignments.Is(q) || ApprovalCheckingAssignments.Is(q) || ReviewReworkAssignments.Is(q) || AdvancedAssignments.Is(q)));
    ...
    public virtual IQueryable<Sungero.Workflow.IAssignmentBase> OnSigningDataQuery(IQueryable<Sungero.Workflow.IAssignmentBase> query)
    {
      ...
      var result = query
        .Where(a => (Docflow.ApprovalSigningAssignments.Is(a) && Docflow.ApprovalSigningAssignments.As(a).IsConfirmSigning != true)
               || DocflowApproval.SigningAssignments.Is(a));
    ...
    public virtual IQueryable<Sungero.Workflow.IAssignmentBase> OnReviewDataQuery(IQueryable<Sungero.Workflow.IAssignmentBase> query)
    {
      ...
      var result = query
        .Where(a => showIncoming && (RecordManagement.ReviewManagerAssignments.Is(a) ||
                                     ...
                                     RecordManagement.DocumentReviewAssignments.Is(a) ||
