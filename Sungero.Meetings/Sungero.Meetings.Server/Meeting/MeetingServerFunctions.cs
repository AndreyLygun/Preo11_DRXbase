...
namespace Sungero.Meetings.Server
{
  partial class MeetingFunctions
  {
    ...
    public List<IElectronicDocument> GetMeetingDocuments(string relationName)
    {
      ...
      if (relationName == string.Empty)
      {
        foreach (var document in documents)
          documentList.AddRange(document.Relations.GetRelatedAndRelatedFromDocuments().ToList());
      }
      else
      {
        foreach (var document in documents)
          documentList.AddRange(document.Relations.GetRelatedAndRelatedFromDocuments(relationName).ToList());
      }
    ...
    /// <summary>
    /// Построить модель контрола состояния совещания.
    /// </summary>
    ...
    public Sungero.Core.StateView GetStateView()
    {
      ...
      // Проверить, что количество поручений не превышает максимально возможное число блоков, которое можно вывести в предметном отображении.
      var totalBlocksCount = actionItems.Distinct().Count();
