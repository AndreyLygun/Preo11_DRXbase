...
namespace Sungero.Docflow.Client
{
  partial class ApprovalReworkAssignmentActions
  {
    public virtual void Forward(Sungero.Workflow.Client.ExecuteResultActionArgs e)
    {
      ...
      // Выбрать, кому переадресовать, или отменить действие.
      if (!Functions.ApprovalReworkAssignment.ShowForwardingDialog(_obj))
        e.Cancel();
      
      // Назначить права на вложения или отменить действие.
      var grantRightsResult = Functions.Module.ShowDialogGrantAccessRights(_obj,
                                                                           _obj.OtherGroup.All.ToList(),
                                                                           new List<IRecipient>() { _obj.ForwardPerformer });
      if (grantRightsResult == false)
        e.Cancel();
    }
    ...
    public virtual void AbortApprovingAction(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      {
        var task = _obj.Task;
        ((Sungero.Domain.Shared.IExtendedEntity)task).Params.Add(Constants.ApprovalTask.AbortedFromReworkAssignmentParameterName, true);
        task.Abort();
