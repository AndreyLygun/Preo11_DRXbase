...
namespace Sungero.Docflow.Client
{
  partial class OutgoingDocumentBaseFunctions
  {
    ...
    /// <summary>
    /// Получить имя первого адресата.
    /// </summary>
    /// <returns>Имя адресата.</returns>
    public virtual string GetFirstAddresseeName()
    {
      ...
      if (addresseeRaw.Addressee != null)
      {
        ...
        {
          CommonLibrary.PersonFullName contactName = null;
          CommonLibrary.PersonFullName.TryParse(addresseeRaw.Addressee.Name, out contactName);
          addresseeName = contactName == null
            ? addresseeRaw.Addressee.Name
            : Parties.PublicFunctions.Module.GetSurnameAndInitialsInTenantCulture(contactName.FirstName, contactName.MiddleName, contactName.LastName);
        }
