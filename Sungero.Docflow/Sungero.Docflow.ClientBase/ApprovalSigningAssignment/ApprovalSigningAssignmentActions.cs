...
namespace Sungero.Docflow.Client
{
  partial class ApprovalSigningAssignmentActions
  {
    ...
    public virtual void Sign(Sungero.Workflow.Client.ExecuteResultActionArgs e)
    {
      ...
      var attachments = _obj.Attachments.Where(a => _obj.AttachmentsInfo.Any(i => i.IsLinkedTo(a) && i.GroupName == nameof(_obj.OtherGroup)));
      var accessRightsGranted = Functions.Module.ShowDialogGrantAccessRights(_obj, attachments.ToList());
      if (accessRightsGranted == false)
        e.Cancel();
      ...
      if (!confirmationAccepted && accessRightsGranted == null &&
          !Functions.ApprovalTask.ConfirmCompleteAssignment(confirmationMessage, Constants.ApprovalTask.ApprovalSigningAssignmentConfirmDialogID.Sign, true))
        e.Cancel();
