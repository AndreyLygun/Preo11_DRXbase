...
namespace Sungero.Docflow.Client
{
  partial class ApprovalTaskFunctions
  {
    ...
    /// <param name="isSigning">Признак того, что документ проходит этап подписания.</param>
    /// <returns>True, если запрос был подтверждён.</returns>
    public static bool ConfirmCompleteAssignment(string confirmationMessage, string dialogId, bool isSigning = false)
    {
      return ConfirmCompleteAssignment(false, true, confirmationMessage, dialogId, isSigning);
    }
    ...
    public virtual void AbortAsyncProcessingNotify()
    {
      ...
      var needSetStateAsync = !document.AccessRights.CanUpdate() || Locks.GetLockInfo(document).IsLocked;
