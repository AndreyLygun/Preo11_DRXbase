...
namespace Sungero.Docflow.Client
{
  ...
  {
    public virtual void CreateCancellationAgreement(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      var errorMessage = Exchange.PublicFunctions.Module.Remote.ValidateBeforeCreatingCancellationAgreement(_obj.Id);
      if (!string.IsNullOrWhiteSpace(errorMessage))
        Dialogs.NotifyMessage(errorMessage);
      else
        Functions.OfficialDocument.ShowCreateCancellationAgreementDialog(_obj);
    }

    public virtual bool CanCreateCancellationAgreement(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return _obj.ExchangeState != null && _obj.ExchangeState != ExchangeState.Terminated && _obj.ExchangeState != ExchangeState.Obsolete &&
        _obj.ExchangeState != ExchangeState.Rejected && !_obj.State.IsChanged;
    }

    public virtual void ShowComparisonResult(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      if (!Docflow.PublicFunctions.Module.Remote.IsModuleAvailableByLicense(Commons.PublicConstants.Module.IntelligenceGuid))
      {
        Dialogs.NotifyMessage(OfficialDocuments.Resources.NoLicenseToCompareDocuments);
      }
      else
      {
        var allResults = Functions.OfficialDocument.Remote.GetComparisonResults(_obj);
        if (allResults.Any())
        {
          var comparisonInfo = allResults.First();
          if (comparisonInfo.ProcessingStatus == Docflow.DocumentComparisonInfo.ProcessingStatus.Compared ||
        ...
        {
          Dialogs.NotifyMessage(OfficialDocuments.Resources.ComparisonResultsNotFound);
        }
      }
    }

    public virtual bool CanShowComparisonResult(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return _obj.HasVersions && !_obj.State.IsInserted && !_obj.IsEncrypted;
    }

    public virtual void CompareDocuments(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      if (!Docflow.PublicFunctions.Module.Remote.IsModuleAvailableByLicense(Commons.PublicConstants.Module.IntelligenceGuid))
        Dialogs.NotifyMessage(OfficialDocuments.Resources.NoLicenseToCompareDocuments);
      else if (string.IsNullOrEmpty(Docflow.PublicFunctions.Module.GetArioUrl()))
        Dialogs.NotifyMessage(SmartProcessingSettings.Resources.ArioConnectionError);
      else
        Functions.OfficialDocument.ShowCompareDocumentsDialog(_obj);
    }

    public virtual bool CanCompareDocuments(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return _obj.HasVersions && !_obj.State.IsInserted && !_obj.State.IsChanged && !_obj.IsEncrypted;
    }

    public virtual void CompareVersions(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      if (!Docflow.PublicFunctions.Module.Remote.IsModuleAvailableByLicense(Commons.PublicConstants.Module.IntelligenceGuid))
        Dialogs.NotifyMessage(OfficialDocuments.Resources.NoLicenseToCompareDocuments);
      else if (string.IsNullOrEmpty(Docflow.PublicFunctions.Module.GetArioUrl()))
        Dialogs.NotifyMessage(SmartProcessingSettings.Resources.ArioConnectionError);
      else if (_obj.Versions.Count < 2)
        Dialogs.NotifyMessage(OfficialDocuments.Resources.DocumentShouldHaveAtLeastTwoVersions);
      else
        Functions.OfficialDocument.ShowCompareVersionsDialog(_obj);
    }

    public virtual bool CanCompareVersions(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return _obj.HasVersions && !_obj.State.IsInserted && !_obj.State.IsChanged && !_obj.IsEncrypted;
    }
    ...
    public virtual void ChangeDocumentType(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      if (Functions.OfficialDocument.Remote.HasSpecifiedTypeRelations(_obj))
      {
        ...
        {
          var relatedDocuments = _obj.Relations.GetRelatedAndRelatedFromDocuments().ToList();
    ...
    public virtual void ConvertToPdf(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      Functions.OfficialDocument.LogPdfConversion(_obj, version.Id, "Start action");
      
      // При асинхронном преобразовании не отправлять на обработку повторно.
      long convertingVersionIdParamValue = -1;
      bool conversionInProcess = !e.Params.TryGetValue(Constants.OfficialDocument.ConvertingVersionId, out convertingVersionIdParamValue) ||
        convertingVersionIdParamValue != version.Id;
      
      if (!conversionInProcess)
      {
        e.AddInformation(Sungero.Docflow.OfficialDocuments.Resources.ConversionAlreadyInProcess);
        return;
      }
      
      if (!Functions.OfficialDocument.Remote.IsExchangeDocument(_obj, version.Id))
      {
        /* Сначала валидируем данные для проверки отметок, так как они более проритетны, а уже потом валидируем сам документ.
         * Для документов МКДО не валидируем, т.к. они преобразуются старым алгоритмом.
         */
        var marksDataValidationResult = Functions.OfficialDocument.Remote.ValidateMarksDataBeforeConversion(_obj, version.Id);
        if (marksDataValidationResult.HasErrors)
        {
          Dialogs.NotifyMessage(marksDataValidationResult.ErrorMessage);
          Functions.OfficialDocument.LogPdfConversion(_obj, version.Id, marksDataValidationResult.ErrorMessage);
          return;
        }
      }
      
      var documentValidationResult = Functions.OfficialDocument.Remote.ValidateDocumentBeforeConversion(_obj, version.Id);
      if (documentValidationResult.HasErrors)
      {
        Dialogs.NotifyMessage(documentValidationResult.ErrorMessage);
        Functions.OfficialDocument.LogPdfConversion(_obj, version.Id, documentValidationResult.ErrorMessage);
        return;
      }
      
      var conversionResult = Structures.OfficialDocument.ConversionToPdfResult.Create();
      // Идём по старому алгоритму, если явно указан соответствующий признак в Sungero_Docflow_Params, либо это документ МКДО.
      var useObsoletePdfConversion = PublicFunctions.Module.Remote.UseObsoletePdfConversion() || documentValidationResult.IsExchangeDocument;
      if (useObsoletePdfConversion)
      {
        Functions.OfficialDocument.LogPdfConversion(_obj, version.Id, "UseObsoletePdfConversion");
        conversionResult = Sungero.Docflow.Functions.OfficialDocument.Remote.ConvertToPdfWithSignatureMark(_obj);
      }
      else
      {
        ...
        {
          Functions.OfficialDocument.LogPdfConversion(_obj, _obj.LastVersion.Id, "Convert interactively");
          conversionResult = Functions.OfficialDocument.Remote.ConvertToPdfWithMarks(_obj, _obj.LastVersion.Id);
          conversionResult.IsFastConvertion = true;
        }
        else
        {
          Functions.OfficialDocument.LogPdfConversion(_obj, _obj.LastVersion.Id, "Start async conversion");
          Functions.OfficialDocument.Remote.CreateConvertToPdfAndAddSignatureMarkAsyncHandler(_obj, _obj.LastVersion.Id);
          conversionResult.IsOnConvertion = true;
          conversionResult.HasErrors = false;
        }
      }
      
      if (conversionResult.IsOnConvertion)
      ...
      if (!conversionResult.HasErrors && conversionResult.IsFastConvertion)
      {
      ...
      {
        Dialogs.NotifyMessage(conversionResult.ErrorMessage);
        return;
      }
      
      if (Sungero.Docflow.Functions.OfficialDocument.Remote.IsExchangeDocument(_obj, _obj.LastVersion.Id))
      {
    ...
    {
      var related = _obj.Relations.GetRelatedAndRelatedFromDocuments().ToList();
      related.Show();
    }
    ...
    public virtual void ReturnFromCounterparty(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      if (result == signed || result == notSigned)
      {
        ...
        foreach (var row in employeeTracking)
        {
          ...
          Functions.OfficialDocument.WriteTrackingLog("ReturnFromCounterparty. Update tracking after return from counterparty action.", row);
        }
        _obj.Save();
      ...
      if (result == openAssignments)
      {
        ...
        try
        {
          ...
          assignments = Functions.OfficialDocument.Remote.GetReturnAssignments(returnTasks);
        }
        catch (Exception)
        {
          Dialogs.NotifyMessage(Docflow.Resources.JobToReturnNotFound);
    ...
    public virtual void ReturnDocument(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      if (result == returnDocument)
      {
        ...
        foreach (var row in employeeTracking)
        {
          ...
          Functions.OfficialDocument.WriteTrackingLog("Official Document. Document returned.", row);
        }
    ...
    {
      var createdTasks = DocflowApproval.PublicFunctions.Module.Remote.GetDocumentFlowTasks(_obj);
      if (createdTasks.Any())
      ...
      var task = DocflowApproval.PublicFunctions.Module.Remote.CreateDocumentFlowTask(_obj);
      task.Show();
      e.CloseFormAfterAction = true;
    }
    ...
    {
      return Functions.OfficialDocument.CanExecuteSendAction(_obj, _obj.Info.Actions.SendForDocumentFlow);
    }
  }
