...
namespace Sungero.Docflow.Client
{
  partial class FreeApprovalAssignmentActions
  {
    ...
    {
      if (!Functions.FreeApprovalAssignment.ShowForwardingDialog(_obj))
        e.Cancel();
      
      if (Functions.Module.ShowDialogGrantAccessRights(_obj, _obj.OtherGroup.All.ToList(), new List<IRecipient> { _obj.Addressee }) == false)
        e.Cancel();
    }
