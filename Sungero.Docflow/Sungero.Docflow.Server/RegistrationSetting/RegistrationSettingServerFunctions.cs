...
namespace Sungero.Docflow.Server
{
  partial class RegistrationSettingFunctions
  {
    ...
    public static List<IRegistrationSetting> GetDoubleSettings(IRegistrationSetting setting)
    {
      ...
      if (setting.Status == CoreEntities.DatabookEntry.Status.Closed)
        return conflictedSettings;
      ...
      {
        conflictedSettings.AddRange(allSettings.Where(s => s.DocumentKinds.Any(k => Equals(k.DocumentKind, documentKind.DocumentKind))).ToList());
      }
      ...
      if (setting.BusinessUnits.Any())
      {
        ...
        {
          conflictedSettings.AddRange(allSettings.Where(s => s.BusinessUnits.Any(b => Equals(b.BusinessUnit, unit.BusinessUnit))).ToList());
        }
      }
      else
      {
      ...
      if (setting.Departments.Any())
      {
        ...
        {
          conflictedSettings.AddRange(allSettings.Where(s => s.Departments.Any(d => Equals(d.Department, department.Department))).ToList());
        }
      }
      else
      {
