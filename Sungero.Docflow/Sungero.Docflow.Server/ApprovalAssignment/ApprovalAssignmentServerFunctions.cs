...
namespace Sungero.Docflow.Server
{
  partial class ApprovalAssignmentFunctions
  {
    ...
    {
      var assignments = Docflow.Functions.Module.GetParallelAssignments(_obj);
      ...
      foreach (var assignment in materialized)
      {
        ...
        var created = materialized.Count(a => Equals(a.Performer, employee) && Equals(a.ForwardedFrom, assignment) && Equals(a.Status, Status.InProcess));
        if (added < created)
          return false;
      }
    ...
    /// <summary>
    /// Получить всех согласующих текущей итерации.
    /// </summary>
    /// <returns>Все согласующие текущей итерации.</returns>
    [Remote(IsPure = true)]
    public List<IEmployee> GetAllCurrentIterationApprovers()
    {
      var currentApprovers = Docflow.Functions.Module.GetActiveParallelAssignments(_obj)
        .Where(x => Employees.Is(x.Performer))
        .Select(x => Employees.As(x.Performer))
        .ToList();
      
      var futureApprovers = this.GetCurrentIterationEmployeesWithoutAssignment();
      
      return currentApprovers.Union(futureApprovers).Distinct().ToList();
    }
