...
namespace Sungero.Docflow.Server
{
  partial class ApprovalRoleFunctions
  {
    ...
    public override IEmployee GetRolePerformer(IApprovalTask task)
    {
      ...
      if (_obj.Type == Docflow.ApprovalRoleBase.Type.ChiefAccountingOfficer)
        return this.GetPerformerCAO(task);
    ...
    {
      return Functions.Module.GetRegistrar(task.DocumentGroup.OfficialDocuments.FirstOrDefault()) ?? this.GetPerformerInitiator(task);
    }
    ...
    private IEmployee GetPerformerOutDocumentRegister(IApprovalTask task)
    {
      ...
      return Functions.Module.GetOutgoingDocumentsRegistrar(document) ?? this.GetPerformerInitiator(task);
    }
    ...
    private IEmployee GetPerformerPrintResponsible(IApprovalTask task)
    {
      ...
      if (performer == null)
        performer = Functions.Module.GetRegistrar(task.DocumentGroup.OfficialDocuments.FirstOrDefault());
    ...
    /// <summary>
    /// Получить главного бухгалтера.
    /// </summary>
    /// <param name="task">Задача.</param>
    /// <returns>Сотрудник.</returns>
    /// <remarks>Получить главного бухгалтера из НОР. НОР получить из документа.</remarks>
    private IEmployee GetPerformerCAO(IApprovalTask task)
    {
      var document = task.DocumentGroup.OfficialDocuments.FirstOrDefault();
      var businessUnit = document != null ? document.BusinessUnit : null;
      return businessUnit != null ? businessUnit.CAO : null;
    }
