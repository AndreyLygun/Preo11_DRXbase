...
namespace Sungero.Docflow.Server
{
  partial class DocumentRegisterFunctions
  {
    ...
    /// <returns>Журналы.</returns>
    [Remote(IsPure = true)]
    public static List<long> GetDocumentRegisters(IOfficialDocument document, Enumeration? settingType, bool forCurrentUser)
    {
      ...
      var documentRegistersIds = Functions.RegistrationSetting
        .GetAvailableSettingsByParams(document, Docflow.RegistrationSetting.SettingType.Registration)
        .Select(s => s.DocumentRegister.Id);

      // Все журналы, кроме журналов из настроек с типом "Регистрация", не подходящих по параметрам.
      var result = Functions.DocumentRegister.GetFilteredDocumentRegisters(document.DocumentKind.DocumentFlow.Value, true, forCurrentUser)
      ...
      if (settingType == Docflow.RegistrationSetting.SettingType.Reservation)
        result.AddRange(Functions.Module.GetAvailableRegistrationSettings(document, settingType).Select(r => r.DocumentRegister.Id).ToList());
    ...
    /// <returns>Журналы.</returns>
    [Public, Remote(IsPure = true)]
    public static List<long> GetDocumentRegistersIdsByParams(IDocumentKind kind, IBusinessUnit unit, IDepartment department,
                                                             Enumeration? settingType, bool forCurrentUser)
    {
      // Журналы, указанные в активных настройках регистрации с типом "Регистрация".
      var documentRegistersIdsWithSettings = RegistrationSettings
    ...
    [Public, Remote(IsPure = true), Obsolete("Метод не используется с 26.08.2024 и версии 4.11. Используйте метод HasDocumentRegistersByDocument документа OfficialDocument.")]
    ...
    [Public]
    public virtual int GetNextRegistrationNumber(DateTime registrationDate, long leadDocument = 0, long department = 0, long businessUnit = 0)
    {
        ...
        using (var command = connection.CreateCommand())
      {
        ...
        Docflow.PublicFunctions.Module.AddLongParameterToCommand(command, "@docregid", _obj.Id);
        ...
        Docflow.PublicFunctions.Module.AddLongParameterToCommand(command, "@leaddoc", leadDocument);
        ...
        Docflow.PublicFunctions.Module.AddLongParameterToCommand(command, "@newdep", department);
        Docflow.PublicFunctions.Module.AddLongParameterToCommand(command, "@newbusinessunit", businessUnit);
    ...
    /// <param name="date">Дата.</param>
    /// <returns>Порядковый номер.</returns>
    public virtual int? GetNextIndex(IOfficialDocument document, DateTime date)
    {
      ...
      var documentsList = this.GetOtherDocumentsInPeriodBySections(document, date)
        ...
        .Where(d => leadDocumentId == 0 || d.LeadingDocument != null && Equals(d.LeadingDocument.Id, leadDocumentId))
        .Where(d => departmentId == 0 || d.Department != null && Equals(d.Department.Id, departmentId))
        .Where(d => businessUnitId == 0 || d.BusinessUnit != null && Equals(d.BusinessUnit.Id, businessUnitId))
        .Select(d => d.Index)
        .ToList();
    ...
    /// <returns>Порядковый номер.</returns>
    [Obsolete("Метод не используется с 02.07.2024 и версии 4.11. Используйте метод GetNextIndex с меньшим числом параметров.")]
    ...
    [Public, Remote(IsPure = true)]
    public virtual string GetNextNumber(IOfficialDocument document, DateTime date)
    {
      var index = Functions.DocumentRegister.GetNextIndex(_obj, document, date).ToString();
      return Functions.DocumentRegister.GenerateRegistrationNumber(_obj, document, date, index);
    }
    ...
    /// <returns>Регистрационный номер.</returns>
    [Remote(IsPure = true)]
    [Obsolete("Метод не используется с 02.07.2024 и версии 4.11. Используйте метод GetNextNumber с меньшим числом параметров.")]
    public virtual string GetNextNumber(DateTime date, long leadDocumentId, IOfficialDocument document, string leadingDocumentNumber,
                                        long departmentId, long businessUnitId, string caseFileIndex, string docKindCode, string indexLeadingSymbol)
    {
    ...
    [Public, Remote(IsPure = true)]
    public virtual bool IsRegistrationNumberUnique(IOfficialDocument document, string registrationNumber, int index, DateTime registrationDate)
    {
      ...
      AccessRights.AllowRead(
        () =>
        {
          ...
          if (index == 0)
            ...
            index = useObsoleteRegNumberGeneration
              ? Functions.DocumentRegister.GetIndexFromRegistrationNumber(_obj, registrationDate, registrationNumber,
              ...
              : Functions.DocumentRegister.GetIndexFromRegistrationNumber(_obj, document, registrationDate, registrationNumber, checkRegistrationNumberUnique);
          ...
          {
            // Параметр функции "Искать корректировочный постфикс" = true, если необходимо проверять рег. номер на уникальность.
            if (Functions.DocumentRegister.AreRegistrationNumbersEqual(_obj, document, registrationDate, number,
    ...
    /// <returns>True, если номер уникален, и false, если есть документы с таким же номером.</returns>
    [Public, Remote(IsPure = true)]
    [Obsolete("Метод не используется с 02.07.2024 и версии 4.11. Используйте метод IsRegistrationNumberUnique с меньшим числом параметров.")]
    public virtual bool IsRegistrationNumberUnique(IOfficialDocument document, string registrationNumber, int index,
                                                   ...
                                                   string caseFileIndex, string docKindCode, string counterpartyCode, long leadDocumentId)
    {
      var checkRegistrationNumberUnique = Functions.OfficialDocument.CheckRegistrationNumberUnique(document);
    ...
    {
      return Company.PublicFunctions.Department.Remote.GetDepartmentsWithNullCode().Any();
    }
    ...
    {
      return Company.PublicFunctions.BusinessUnit.Remote.GetBusinessUnitsWithNullCode().Any();
    }
    
    /// <summary>
    /// Проверить, есть ли хоть одна категория договора с незаполненным кодом.
    /// </summary>
    /// <returns>True, если есть хоть одна категория договора без кода.</returns>
    [Remote(IsPure = true)]
    public static bool HasContractCategoriesWithNullCode()
    {
      return Contracts.PublicFunctions.ContractCategory.Remote.GetContractCategoriesWithNullCode().Any();
    }
