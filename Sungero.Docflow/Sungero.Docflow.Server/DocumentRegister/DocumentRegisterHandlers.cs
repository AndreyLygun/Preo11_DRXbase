...
namespace Sungero.Docflow
{
  ...
  partial class DocumentRegisterServerHandlers
  {
    ...
    public override void BeforeSave(Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      var numberElements = format.Where(f => f.Element == FormatElement.Number);
      ...
      var beginningOfLine = format.Where(f => f.Element == FormatElement.BegginingOfLine).FirstOrDefault();
      ...
      if (Functions.DocumentRegister.NumberFormatContains(_obj, FormatElement.RegistrPlace) && _obj.RegistrationGroup == null)
        e.AddError(DocumentRegisters.Resources.ForDocumentNumberMustBeAttendRegisterGroup);
      
      // Проверить, что код категории используется только для договорного документопотока.
      var hasCategoryCode = Functions.DocumentRegister.NumberFormatContains(_obj, FormatElement.CategoryCode);
      if (hasCategoryCode && _obj.DocumentFlow != DocumentRegister.DocumentFlow.Contracts)
        e.AddError(DocumentRegisters.Resources.CategoryCodeOnlyForContracts);
    ...
    public override void Created(Sungero.Domain.CreatedEventArgs e)
    {
      ...
      if (!_obj.NumberFormatItems.Any())
      {
        ...
        beginningOfLine.Element = FormatElement.BegginingOfLine;
        
        var number = _obj.NumberFormatItems.AddNew();
        number.Number = 1;
        number.Element = FormatElement.Number;
      }
