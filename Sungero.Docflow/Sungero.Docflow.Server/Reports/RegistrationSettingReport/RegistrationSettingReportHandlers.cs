...
namespace Sungero.Docflow
{
  partial class RegistrationSettingReportServerHandlers
  {
        ...
        foreach (var flow in flows)
        {
          foreach (var kind in kinds.Where(k => Equals(k.DocumentFlow, flow)))
          {
            ...
            foreach (var setting in subSettings)
            {
              ...
              var line = GetDefaultLine(unit, flow, flows.IndexOf(flow), kind, reportSessionId);
              ...
              line.NumberExample = setting.DocumentRegister.ValueExample;
            ...
            foreach (var register in registers)
            {
              ...
              line.SettingType = GetSettingType(kind, null);
              line.NumberExample = register.ValueExample;
    ...
    private static Structures.RegistrationSettingReport.TableLine GetDefaultLine(Company.IBusinessUnit unit, Enumeration flow,
                                                                                 int? flowIndex, IDocumentKind kind, string sessionId)
    {
      ...
      line.DocumentFlowIndex = flowIndex;
      line.DocumentKind = kind.Name;
      line.ReportSessionId = sessionId;
