...
namespace Sungero.Docflow.Server
{
  partial class DocumentKindFunctions
  {
    ...
    public bool IsExchangeNativeDocumentKind()
    {
      return Domain.ModuleFunctions.GetAllExternalLinks(l => l.ExternalSystemId == Constants.Module.InitializeExternalLinkSystem && Equals(l.EntityId, _obj.Id) &&
                                                        (l.ExternalEntityId == Constants.DocumentKind.ContractStatementKind.ToString() ||
                                                         ...
                                                         l.ExternalEntityId == Constants.DocumentKind.CancellationAgreementKind.ToString() ||
                                                         l.ExternalEntityId == Contracts.PublicConstants.Module.Initialize.IncomingInvoiceKind.ToString()))
        .Any();
    }
    ...
    {
      return GetDocumentKindsWithNullCode().Any();
    }
    ...
    /// <summary>
    /// Получить виды документов с незаполненным кодом.
    /// </summary>
    /// <returns>Виды документов с незаполненным кодом.</returns>
    [Remote(IsPure = true)]
    public static IQueryable<IDocumentKind> GetDocumentKindsWithNullCode()
    {
      return DocumentKinds.GetAll().Where(k => k.Status == Status.Active && k.Code == null);
    }
