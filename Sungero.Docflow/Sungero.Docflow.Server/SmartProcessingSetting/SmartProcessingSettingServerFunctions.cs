...
namespace Sungero.Docflow.Server
{
  partial class SmartProcessingSettingFunctions
  {
    ...
    public virtual List<Docflow.Structures.SmartProcessingSetting.IClassifier> GetArioClassifiersPublic()
    {
      ...
      {
        Logger.Error("Error getting the list of classifiers from Ario.", ex);
      }
    ...
    public virtual List<string> GetArioSupportedLanguages()
    {
      ...
      {
        Logger.Error("Error getting the supported languages list from Ario.", ex);
    ...
    {
      var arioUrl = _obj.ArioUrl;
      if (string.IsNullOrEmpty(arioUrl))
        return false;
      try
      {
        var arioConnector = ArioExtensions.ArioConnector.Get(arioUrl);
        return arioConnector.CheckConnection();
      }
      catch (Exception ex)
      {
        Logger.Error("Error connecting to Ario.", ex);
      }
      return false;
    }
    ...
    public Structures.SmartProcessingSetting.LoginResult Login(string password, bool passwordIsEncrypted)
    {
      ...
      {
        if (string.Equals(ex.Message, Constants.SmartProcessingSetting.ArioInvalidLoginPassword))
          loginResult.Error = Sungero.Docflow.SmartProcessingSettings.Resources.ArioLoginError;
        else
