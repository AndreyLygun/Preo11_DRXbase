...
namespace Sungero.Docflow.Server
{
  partial class OutgoingDocumentBaseFunctions
  {
    ...
    public static string AddresseeAppeal(IOutgoingDocumentBase document)
    {
      ...
      else if (addressee != null && addressee.Person != null && addressee.Person.Sex != null)
    ...
    public static IOfficialDocument Contract(IOutgoingDocumentBase document)
    {
      ...
      var contracts = document.Relations.GetRelatedFromDocuments(Constants.Module.CorrespondenceRelationName).Where(d => d.TypeDiscriminator == contractTypeGuid);
    ...
    public override bool HasSpecifiedTypeRelations()
    {
      ...
      AccessRights.AllowRead(
        () =>
        {
          hasSpecifiedTypeRelations = IncomingDocumentBases.GetAll().Any(x => x.InResponseToDocuments.Any(d => Equals(d.Document, _obj)));
        });
    ...
    [Public]
    public override void FillSentDate(ICounterparty correspondent)
    {
      if (_obj.IsManyAddressees == true)
      {
        var addressees = _obj.Addressees.Where(a => Equals(a.Correspondent, correspondent)).ToList();
        foreach (var addressee in addressees)
          addressee.SentDate = Calendar.Now;
      }
      else
        _obj.SentDate = Calendar.Now;
    }
    
    public override List<Sungero.ExchangeCore.IExchangeService> GetExchangeServices()
    {
      if (_obj.IsManyAddressees == true)
        return new List<Sungero.ExchangeCore.IExchangeService>();
      
      return base.GetExchangeServices();
    }
  }
}