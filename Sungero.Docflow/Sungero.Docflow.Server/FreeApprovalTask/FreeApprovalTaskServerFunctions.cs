  ...
  partial class FreeApprovalTaskFunctions
  {
    ...
    public Sungero.Core.StateView GetStateView()
    {
      ...
      foreach (var iteration in iterations)
      {
        ...
        var approvalAssignments = assignments.Where(a => FreeApprovalAssignments.Is(a) || DocflowApproval.EntityApprovalAssignments.Is(a));
        ...
        if (parentBlock != null)
        {
          ...
          if (approvalAssignments.Any(a => a.Result == Docflow.FreeApprovalAssignment.Result.ForRework ||
                                      a.Result == DocflowApproval.EntityApprovalAssignment.Result.ForRework))
          {
          ...
          else if (approvalAssignments.All(x => x.Result == DocflowApproval.EntityApprovalAssignment.Result.Approved ||
                                           x.Result == DocflowApproval.EntityApprovalAssignment.Result.WithSuggestions ||
                                           x.Result == DocflowApproval.EntityApprovalAssignment.Result.Forward))
          {
    ...
    public void AddSendForApprovalBlock(Sungero.Core.StateBlock parentBlock, IAssignment assignment, bool hasReworkBefore, bool hasRestartBefore)
    {
      ...
      Docflow.PublicFunctions.OfficialDocument
        .AddUserActionBlock(parentBlock, assignment.Performer, textTemplate, assignment.Modified.Value, _obj, comment, assignment.CompletedBy);
    }
    ...
    public void AddAssignmentsBlocks(Sungero.Core.StateBlock parentBlock, Sungero.Core.StateBlock taskBlock, List<IAssignment> assignments)
    {
      foreach (var assignment in assignments)
      {
        ...
        StateBlock block;
        if (parentBlock != null && (FreeApprovalAssignments.Is(assignment) || DocflowApproval.EntityApprovalAssignments.Is(assignment)))
        ...
        if (!FreeApprovalAssignments.Is(assignment) && !DocflowApproval.EntityApprovalAssignments.Is(assignment))
        {
          var blockLabel = string.Empty;
          if (FreeApprovalFinishAssignments.Is(assignment) || DocflowApproval.AdvancedAssignments.Is(assignment))
            blockLabel = FreeApprovalTasks.Resources.StateViewCompleteApprovalAssignment;
          else if (FreeApprovalReworkAssignments.Is(assignment) || DocflowApproval.EntityReworkAssignments.Is(assignment))
    ...
    public void SetIcon(Sungero.Core.StateBlock block, IAssignment assignment)
    {
      ...
      if (FreeApprovalAssignments.Is(assignment) ||
          DocflowApproval.EntityApprovalAssignments.Is(assignment))
      ...
      if (assignment.Result != null)
      {
        ...
        if (assignment.Result == Docflow.FreeApprovalAssignment.Result.Approved ||
            assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.Approved)
        {
          iconSize = StateBlockIconSize.Small;
        ...
        if (assignment.Result == Docflow.FreeApprovalAssignment.Result.ForRework ||
            assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.ForRework)
        {
          iconSize = StateBlockIconSize.Small;
        ...
        if (assignment.Result == Docflow.FreeApprovalReworkAssignment.Result.Reworked ||
            assignment.Result == DocflowApproval.EntityReworkAssignment.Result.ForReapproval)
        {
        ...
        if (assignment.Result == Docflow.FreeApprovalFinishAssignment.Result.Complete ||
            assignment.Result == DocflowApproval.AdvancedAssignment.Result.Complete)
        {
        ...
        if (assignment.Result == Docflow.FreeApprovalAssignment.Result.Forward ||
            assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.Forward)
        {
          block.AssignIcon(FreeApprovalTasks.Resources.Forward, iconSize);
        }
        
        // Согласовано с замечаниями.
        if (assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.WithSuggestions)
        {
    ...
    public Structures.FreeApprovalTask.AssignmentInfo GetPerformerAndDeadlineAndStatus(IAssignment assignment)
    {
      ...
      if (assignment.Status == Workflow.AssignmentBase.Status.Completed)
      {
        ...
        // Задание на завершение согласования (старая схема).
        ...
        // Согласование (если схема задачи настраивается в проводнике).
        if (DocflowApproval.EntityApprovalAssignments.Is(assignment))
        {
          if (assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.Approved)
            actionLabel = ApprovalTasks.Resources.StateViewEndorsed;
          else if (assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.WithSuggestions)
            actionLabel = ApprovalTasks.Resources.StateViewEndorsedWithSuggestions;
          else if (assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.ForRework)
            actionLabel = ApprovalTasks.Resources.StateViewNotApproved;
          else if (assignment.Result == DocflowApproval.EntityApprovalAssignment.Result.Forward)
            actionLabel = ApprovalTasks.Resources.StateViewForwarded;
          else
            return emptyResult;
        }
        
        // Задание на завершение согласования (если схема задачи настраивается в проводнике).
        if (DocflowApproval.AdvancedAssignments.Is(assignment))
        {
          if (assignment.Result == DocflowApproval.AdvancedAssignment.Result.Complete)
            actionLabel = ApprovalTasks.Resources.StateViewDone;
          else
            return emptyResult;
        }

        // Переадресация на доработке (если схема задачи настраивается в проводнике).
        if (assignment.Result == DocflowApproval.EntityReworkAssignment.Result.Forward && DocflowApproval.EntityReworkAssignments.Is(assignment))
          actionLabel = ApprovalTasks.Resources.StateViewForwarded;
    ...
    /// </summary>
    /// <param name="assignment">Задание, из которого запросили продление срока.</param>
    /// <param name="newDeadline">Новый срок.</param>
    /// <returns>True - если продление срока задачи прошло успешно, иначе - false.</returns>
    public virtual bool ExtendTaskDeadline(IAssignment assignment, DateTime newDeadline)
    {
      if (Functions.Module.CheckDeadline(newDeadline, _obj.MaxDeadline))
      {
    ...
    {
      return _obj.ForApprovalGroup.ElectronicDocuments.FirstOrDefault();
    }
    
    #region Expression-функции для No-Code
    
    /// <summary>
    /// Признак, что есть задания на согласование в работе.
    /// </summary>
    /// <param name="task">Задача на свободное согласование.</param>
    /// <returns>True - если есть задания на согласование в работе, иначе - false.</returns>
    [ExpressionElement("HasApprovalAssignmentsInProcess", "")]
    public static bool HasApprovalAssignmentsInProcess(Sungero.Docflow.IFreeApprovalTask task)
    {
      var assignmentsInWork = DocflowApproval.EntityApprovalAssignments.GetAll(asg => asg.Task.Equals(task) &&
                                                                               asg.TaskStartId == task.StartId &&
                                                                               asg.Status == Sungero.Workflow.AssignmentBase.Status.InProcess);
      return assignmentsInWork.Any();
    }
