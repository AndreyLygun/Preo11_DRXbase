...
namespace Sungero.Docflow.Server
{
  partial class SignatureSettingFunctions
  {
    ...
    {
      return GetActiveSignatureSettings()
    ...
    public static IQueryable<ISignatureSetting> GetSignatureSettings(List<IBusinessUnit> businessUnits, List<IDocumentKind> kinds)
    {
      ...
      return GetActiveSignatureSettings()
        ...
        .Where(s => !s.ValidTill.HasValue || s.ValidTill.Value >= today);
    }
    
    /// <summary>
    /// Получить права подписи подписывающего.
    /// </summary>
    /// <param name="signatory">Подписывающий.</param>
    /// <returns>Права подписи подписывающего или роли, в которую он входит.</returns>
    [Public]
    public static IQueryable<ISignatureSetting> GetSignatureSettings(IEmployee signatory)
    {
      var activeSignatureSettings = GetActiveSignatureSettings();
      return FilterSignatureSettingsBySignatory(activeSignatureSettings, signatory);
    }
    
    /// <summary>
    /// Получить действующие права подписи.
    /// </summary>
    /// <returns>Действующие права подписи.</returns>
    [Public]
    public static IQueryable<ISignatureSetting> GetActiveSignatureSettings()
    {
      return SignatureSettings.GetAll(s => s.Status == Docflow.SignatureSetting.Status.Active);
    }

    /// <summary>
    /// Отфильтровать права подписи по подписывающему.
    /// </summary>
    /// <param name="signatureSettings">Права подписи.</param>
    /// <param name="signatory">Подписывающий.</param>
    /// <returns>Права подписи подписывающего или роли, в которую он входит.</returns>
    [Public]
    public static IQueryable<ISignatureSetting> FilterSignatureSettingsBySignatory(IQueryable<ISignatureSetting> signatureSettings, IEmployee signatory)
    {
      if (signatory == null)
        return Enumerable.Empty<ISignatureSetting>().AsQueryable();
      
      var groupIds = Recipients.OwnRecipientIdsFor(signatory);
      groupIds.Append(signatory.Id);
      
      return signatureSettings.Where(s => groupIds.Contains(s.Recipient.Id));
    }    
