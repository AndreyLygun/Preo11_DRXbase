...
namespace Sungero.Docflow.Shared
{
  partial class MemoFunctions
  {
    ...
    public override void UpdateDocumentApprovalState(Enumeration? newState, long? taskId)
    {
      ...
      // Не меняем статус рассмотренной служебной записки.
      if (currentState == Docflow.Memo.InternalApprovalState.Reviewed)
      {
        Logger.DebugFormat("UpdateApprovalState: {0}document {1} already reviewed.", taskIdText, _obj.Id);
        return;
      }
      
      // Если текущий статус - Подписан, то поменять можно только на Рассмотрен.
      if (currentState == InternalApprovalState.Signed && newState != Docflow.Memo.InternalApprovalState.Reviewed)
      {
    ...
    [Public]
    public override bool TaskAdresseesFieldIsEnabled()
    {      
      return false;
    }
  }
