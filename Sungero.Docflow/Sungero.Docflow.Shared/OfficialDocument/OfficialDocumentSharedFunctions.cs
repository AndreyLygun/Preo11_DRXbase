...
namespace Sungero.Docflow.Shared
{
  partial class OfficialDocumentFunctions
  {
    ...
    {
      return _obj.RegistrationState == RegistrationState.NotRegistered;
    }
    
    public virtual bool NumberingTypeChanged()
    {
      return !Equals(_obj.DocumentKind?.NumberingType, _obj.State.Properties.DocumentKind.OriginalValue?.NumberingType);
    }
    
    public virtual bool IsRegistrable()
    {
      return _obj.DocumentKind?.NumberingType == Docflow.DocumentKind.NumberingType.Registrable;
    }
    ...
    /// </summary>
    /// <returns>Код категории либо пустая строка.</returns>
    [Public]
    ...
    /// </summary>
    /// <param name="clearRegistrationDateAndNumber">Очистить рег. номер и дату.</param>
    ...
    {
      if (clearRegistrationDateAndNumber)
      {
      ...
      // Отменить необходимость регистрации, т.к. рег.данные очищены.
    ...
    /// <param name="tracking">Объект выдачи.</param>
    [Public]
    public static void WriteTrackingLog(string logMessage, IOfficialDocumentTracking tracking)
    {
      Logger.WithLogger(Constants.Module.TrackingLoggerPostfix)
        .Debug(string.Format(logMessage + " DocumentId: {0}. ReturnTask: {1}. ReturnResult: {2}. ReturnDate: {3}.",
                             tracking.OfficialDocument.Id,
                             tracking.ReturnTask?.Id.ToString() ?? "null",
                             tracking.ReturnResult?.ToString() ?? "null",
                             tracking.ReturnDate?.ToString() ?? "null"));
    }
    ...
    public static List<long> GetDocumentRegistersIdsByDocument(IOfficialDocument document, Enumeration? settingType)
    {
      ...
      return Functions.DocumentRegister.Remote.GetDocumentRegisters(document, settingType, true);
    }
    ...
    public virtual bool HasDocumentRegistersByDocument(Enumeration? settingType)
    {
      ...
      return Functions.DocumentRegister.Remote.GetDocumentRegisters(_obj, settingType, true).Any();
    }
    ...
    public virtual void RefreshDocumentForm()
    {
      ...
      // Подрубрика доступна, только если заполнена рубрика.
      _obj.State.Properties.Subtopic.IsEnabled = _obj.Topic != null;
    }
    ...
    public virtual void ChangeDocumentPropertiesAccess(bool isEnabled, bool repeatRegister)
    {
      ...
      else
      {
        ...
        var businessUnitCodeIncludedInNumber = repeatRegister && documentRegister != null &&
          Functions.DocumentRegister.NumberFormatContains(documentRegister, FormatElement.BUCode);
        ...
        var counterpartyCodeIncludedInNumber = repeatRegister && documentRegister != null &&
          Functions.DocumentRegister.NumberFormatContains(documentRegister, FormatElement.CPartyCode);
      ...
      this.ChangeMediumTypePropertyAccess();
    ...
    public virtual void ChangeRegistrationPaneVisibility(bool needShow, bool repeatRegister)
    {
      ...
      var caseFileIncludedInNumber = register != null &&
        Functions.DocumentRegister.NumberFormatContains(register, FormatElement.CaseFile);
      ...
      properties.Medium.IsEnabled = canUpdate;
      properties.Medium.IsVisible = needShow;
      ...
      properties.StoredIn.IsEnabled = canRegister;
      properties.StoredIn.IsVisible = needShow;
      
      properties.PaperCount.IsEnabled = canUpdate;
      properties.PaperCount.IsVisible = needShow;
      
      properties.AddendaPaperCount.IsEnabled = canUpdate;
      properties.AddendaPaperCount.IsVisible = needShow;
    ...
    /// </summary>
    /// <param name="needSetObsolete">Признак установки статуса "Устаревший".</param>
    [Public]
    public virtual void SetDocumentStateAborted(bool needSetObsolete)
    {
      Logger.DebugFormat("SetDocumentStateAborted: set document id = {0} state aborted with setting obsolete = {1}", _obj.Id, needSetObsolete);
      this.UpdateDocumentApprovalState(InternalApprovalState.Aborted, null);
      if (needSetObsolete)
      {
        var isActive = _obj.LifeCycleState == Sungero.Docflow.OfficialDocument.LifeCycleState.Active;
        this.SetObsolete(isActive);
      }
    }
    ...
    {
      var currentState = _obj.InternalApprovalState;
      var taskIdText = taskId != null ? $"Task {taskId}, " : string.Empty;
      
      // Рассмотрен - статус только для служебных записок,
      // по умолчанию заменяем его на Подписан.
      if (newState == Docflow.Memo.InternalApprovalState.Reviewed)
      {
        Logger.DebugFormat("UpdateApprovalState: change state Reviewed -> Signed");
        newState = InternalApprovalState.Signed;
      }
      
      // По умолчанию не меняем статус подписанного документа.
      if (currentState == InternalApprovalState.Signed)
      {
        Logger.DebugFormat("UpdateApprovalState: {0}document {1} already signed.", taskIdText, _obj.Id);
        return;
      }
      
      // В остальных случаях обновляем статус.
      Logger.DebugFormat("UpdateApprovalState: {3}document {0}, {1} -> {2}", _obj.Id, currentState, newState, taskIdText);
      
      if (_obj.InternalApprovalState != newState)
        _obj.InternalApprovalState = newState;
    }
    ...
    {
      if (_obj.LifeCycleState != LifeCycleState.Obsolete)
    ...
    /// </summary>
    /// <param name="documentKind">Актуальный вид документа.</param>
    [Public]
    public virtual void FillDefaultMediumType(IDocumentKind documentKind)
    {
      if (_obj.Medium != null || documentKind == null || documentKind.DefaultMedium == null || Equals(_obj.Medium, documentKind.DefaultMedium))
        return;

      _obj.Medium = documentKind.DefaultMedium;
    }
    ...
    {
      var verificationInProcess = _obj.VerificationState == VerificationState.InProcess;
      if (string.IsNullOrWhiteSpace(_obj.Name) || verificationInProcess)
        return;
      
      var oldNameIsNotAutogenerated = e.OriginalValue == null || e.OriginalValue.GenerateDocumentName != true;
      var newNameIsAutogenerated = e.NewValue != null && e.NewValue.GenerateDocumentName == true;
      if (oldNameIsNotAutogenerated && newNameIsAutogenerated)
      {
    ...
    {
      var oldNameIsAutogenerated = e.OriginalValue != null && e.OriginalValue.GenerateDocumentName == true;
      var newNameIsNotAutogenerated = e.NewValue != null && e.NewValue.GenerateDocumentName != true;
      if (oldNameIsAutogenerated && newNameIsNotAutogenerated)
      {
    ...
    {
      if (e.OriginalValue != null && e.OriginalValue.NumberingType == Sungero.Docflow.DocumentKind.NumberingType.NotNumerable &&
    ...
    {
      return Functions.OfficialDocument.Remote.GetExtensionValidationErrorRemote(_obj, extension);
    }
    ...
    {
      var documentLogPrefix = this.GetPdfConversionDocumentLogPrefix(versionId);
      Logger.Debug($"{documentLogPrefix}. {message}");
    }
    ...
    {
      var documentLogPrefix = this.GetPdfConversionDocumentLogPrefix(versionId);
      Logger.Error($"ConvertToPdf. {documentLogPrefix}. {message}", ex);
    }
    ...
    /// </summary>
    /// <param name="versionId">ИД версии.</param>
    /// <returns>Префикс с информацией о документе для записи в лог.</returns>
    public virtual string GetPdfConversionDocumentLogPrefix(long? versionId)
    {
      return versionId == null ?
        $"ConvertToPdf. Document={_obj.Id}" :
        $"ConvertToPdf. Document={_obj.Id} Version={versionId}";
    }
    ...
    public virtual bool NeedDisableLeadingDocument()
    {
      ...
      var register = _obj.DocumentRegister;
      var leadingNumberIncludedInNumber = register != null &&
        (Functions.DocumentRegister.NumberFormatContains(register, FormatElement.LeadingNumber) ||
    ...
    public virtual bool NeedDisableBusinessUnit()
    {
      ...
      var register = _obj.DocumentRegister;
      var businessUnitIncludedInNumber = register != null &&
        (Functions.DocumentRegister.NumberFormatContains(register, FormatElement.BUCode) ||
         register.NumberingSection == Docflow.DocumentRegister.NumberingSection.BusinessUnit);
    ...
    public virtual bool NeedDisableDepartment()
    {
      ...
      var register = _obj.DocumentRegister;
      var departmentIncludedInNumber = register != null &&
        (Functions.DocumentRegister.NumberFormatContains(register, FormatElement.DepartmentCode) ||
         register.NumberingSection == Docflow.DocumentRegister.NumberingSection.Department);
    ...
    {
      return _obj.Relations.GetRelatedDocuments(Docflow.Constants.Module.AddendumRelationName).ToList()
    ...
    {
      var hasCallContext = CallContext.CalledFrom(ApprovalSigningAssignments.Info)
        || CallContext.CalledFrom(ApprovalReviewAssignments.Info)
        || CallContext.CalledFrom(DocflowApproval.SigningAssignments.Info);
    ...
    /// </summary>
    /// <returns>True - адресаты доступны для редактирования, False - иначе.</returns>
    [Public]
    public virtual bool TaskAdresseesFieldIsEnabled()
    {
      // Виртуальная функция, переопределена в потомках.
      return true;
    }
    ...
    {
      _obj.State.Properties.Medium.IsEnabled = _obj.ExchangeState == null;
    }
