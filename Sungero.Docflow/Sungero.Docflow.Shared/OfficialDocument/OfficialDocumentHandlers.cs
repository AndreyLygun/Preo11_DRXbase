...
namespace Sungero.Docflow
{
  ...
  partial class OfficialDocumentVersionsSharedCollectionHandlers
  {
    ...
    public override void VersionsDeleted(Sungero.Domain.Shared.CollectionPropertyDeletedEventArgs e)
    {
      ...
      if (_obj.LastVersion != null && _deleted.Number > _obj.LastVersion.Number)
      {
        ...
        _obj.LastVersionChanged = Calendar.Now;
      }
      
      if (_obj.LastVersion == null)
        _obj.LastVersionChanged = Calendar.Now;
      
      var marks = Functions.OfficialDocument.Remote.GetVersionMarks(_obj, _deleted.Id);
      foreach (var mark in marks)
        Functions.OfficialDocument.Remote.DeleteVersionMark(_obj, mark);
    }
  ...
  partial class OfficialDocumentSharedHandlers
  {
    ...
    public virtual void DocumentKindChanged(Sungero.Docflow.Shared.OfficialDocumentDocumentKindChangedEventArgs e)
    {
      ...
      if (isTypeChanging)
      {
        Functions.OfficialDocument.SaveOldNameAfterTypeChange(_obj, e);
        Functions.OfficialDocument.FillNameIfNotAutogeneratedAfterTypeChange(_obj, e);
        Functions.OfficialDocument.ResetLifeCycleForNotNumerableDocument(_obj, e);
        ...
        Functions.OfficialDocument.ClearRegistrationData(_obj, !verificationInProcess, e);
      }
      
      var notNumerableDocumentKind = e.NewValue != null && e.NewValue.NumberingType == Docflow.DocumentKind.NumberingType.NotNumerable;
      if (notNumerableDocumentKind)
        Functions.OfficialDocument.ClearRegistrationData(_obj, false, e);

      if (!isTypeChanging && notNumerableDocumentKind)
      {
      ...
      Functions.OfficialDocument.FillDefaultMediumType(_obj, e.NewValue);
    }
