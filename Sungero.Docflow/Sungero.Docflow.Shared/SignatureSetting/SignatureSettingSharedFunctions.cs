...
namespace Sungero.Docflow.Shared
{
  partial class SignatureSettingFunctions
  {
    ...
    [Public]
    public virtual bool IsRecipientPowerOfAttorneyRepresentative()
    {
      ...
      // У эл. доверенности на юр. лицо 003 формата нет представителя.
      var formalizedPowerOfAttorney = FormalizedPowerOfAttorneys.As(_obj.Document);
      if (formalizedPowerOfAttorney != null &&
          formalizedPowerOfAttorney.FormatVersion == Docflow.FormalizedPowerOfAttorney.FormatVersion.Version003 &&
          formalizedPowerOfAttorney.Representatives.Any(r => r.AgentType == PowerOfAttorneyBaseRepresentatives.AgentType.LegalEntity))
        return true;

      var powerOfAttorney = PowerOfAttorneyBases.As(_obj.Document);
      var person = Company.Employees.As(_obj.Recipient)?.Person;
      if (person != null && powerOfAttorney.Representatives.Any(r => Equals(person, r.IssuedTo) || Equals(person, r.Agent)))
        return true;
