...
namespace Sungero.Docflow.Shared
{
  public class ModuleFunctions
  {
    ...
    #region Конкурентные задания
    
    /// <summary>
    /// Взято ли задание в работу.
    /// </summary>
    /// <param name="assignment">Задание.</param>
    /// <returns>True, если задание взято в работу или оно не конкурентное.</returns>
    [Public]
    public static bool IsWorkStarted(IAssignment assignment)
    {
      return !assignment.IsCompetitive.GetValueOrDefault() || assignment.IsCompetitiveWorkStarted.GetValueOrDefault();
    }
    
    /// <summary>
    /// Задание конкурентное.
    /// </summary>
    /// <param name="assignment">Задание.</param>
    /// <returns>True, если задание конкурентное.</returns>
    [Public]
    public static bool IsCompetitive(IAssignment assignment)
    {
      return assignment.IsCompetitive.GetValueOrDefault();
    }
    
    /// <summary>
    /// Взято ли конкурентное задание в работу.
    /// </summary>
    /// <param name="assignment">Задание.</param>
    /// <returns>True, если конкурентное задание взято в работу. False, если не взято или не конкурентное.</returns>
    [Public]
    public static bool IsCompetitiveWorkStarted(IAssignment assignment)
    {
      return assignment.IsCompetitiveWorkStarted.GetValueOrDefault();
    }
    ...
    [Obsolete("Метод не используется с 07.02.2024 и версии 4.10, так как больше не актуален.")]
    ...
    public virtual void SynchronizeAddendaAndAttachmentsGroup(Sungero.Workflow.Interfaces.IWorkflowEntityAttachmentGroup group, IElectronicDocument document)
    {
      ...
      var documentAddenda = document.Relations.GetRelatedDocuments(Docflow.Constants.Module.AddendumRelationName).ToList()
    ...
    [Public]
    public virtual List<IElectronicDocument> GetAllAddenda(IElectronicDocument document)
    {
      return document.Relations.GetRelatedDocuments(Docflow.Constants.Module.AddendumRelationName).ToList();
    }
    ...
    {
      var allAddenda = this.GetAllAddenda(document);
      
      var obsoleteDocuments = allAddenda
        .OfType<IOfficialDocument>()
        .Where(x => Functions.OfficialDocument.IsObsolete(x));
      
      return allAddenda
        .Except(obsoleteDocuments)
        .ToList();
    }
    ...
    /// <summary>
    /// Проверить, что для задачи схема настраивается не в проводнике.
    /// </summary>
    /// <param name="task">Задача.</param>
    /// <returns>True - если схема задачи не настраивается в проводнике, иначе false.</returns>
    [Public]
    public virtual bool IsTaskUsingOldScheme(Sungero.Workflow.ITask task)
    {
      return task.ProcessKind == null;
    }
    ...
    public virtual long? GetDocumentIdFromHistoryComment(string comment)
    {
      ...
      var number = commentParts.Length >= Constants.Module.DocumentIdCommentPosition
        ...
        : string.Empty;
      long documentId;
      if (!long.TryParse(number, out documentId))
        return null;
    ...
    public virtual string GetAmountWithCurrencyInWords(double amount, Commons.ICurrency currency)
    {
      ...
      var decimalValue = this.GetDecimalValueStringWithTwoDigits(amount);
      var currencyName = this.GetCurrencyNameForIntegerValuePart(currency, amount);
      var moneyAmountInWords = this.GetAmountWithCurrencyInWordsWithoutDecimalValue(amount, currency);
      
      return string.Format(Constants.Module.MoneyAmountWithCurrencyTemplate, decimalValue, currencyName, moneyAmountInWords);
    }
    ...
    public virtual string GetAmountWithCurrencyInWordsWithoutDecimalValue(double amount, Commons.ICurrency currency)
    {
      ...
      var fractionalPart = (int)Math.Round((amount - integerPart) * 100);
      
      var currencyName = this.GetCurrencyNameForIntegerValuePart(currency, amount);
      var currencyFractionalPartName = this.GetCurrencyNameForFractionalValuePart(currency, amount);
      
      return string.Format(Constants.Module.MoneyAmountWithCurrencyWithoutDecimalValueTemplate,
                           StringUtils.NumberToWords(integerPart).Capitalize(),
                           currencyName,
                           fractionalPart.ToString(Constants.Module.DecimalValueFractionalPartFormat),
                           currencyFractionalPartName);
    }
    ...
    /// <returns>Строка содержащая десятичное значение числа с двумая знаками после запятой.</returns>
    [Public]
    public virtual string GetDecimalValueStringWithTwoDigits(double value)
    {
      var integerPart = (long)Math.Truncate(value);
      var fractionalPart = (int)Math.Round((value - integerPart) * 100);
      return string.Format(Constants.Module.DecimalValueStringTemplate,
                           integerPart.ToString(Constants.Module.DecimalValueIntegerPartFormat),
                           fractionalPart.ToString(Constants.Module.DecimalValueFractionalPartFormat));
    }
    ...
    /// <returns>Наименование валюты.</returns>
    public virtual string GetCurrencyNameForIntegerValuePart(Commons.ICurrency currency, double value)
    {
      ...
      return currencyShortName.EndsWith(Constants.Module.ShortNameEnd)
        ? currencyShortName
        : StringUtils.NumberDeclension(integerPart,
                                       currencyShortName,
                                       Sungero.Core.CaseConverter.ConvertCurrencyNameToTargetDeclension(currencyShortName, Core.DeclensionCase.Genitive),
                                       Sungero.Core.CaseConverter.ConvertCurrencyNameToTargetDeclension(currencyShortName.Pluralize(), Core.DeclensionCase.Genitive));
    }
    ...
    public virtual string GetCurrencyNameForFractionalValuePart(Commons.ICurrency currency, double value)
    {
      ...
      return currencyFractionName.EndsWith(Constants.Module.ShortNameEnd)
        ? currencyFractionName
        : StringUtils.NumberDeclension(fractionalPart,
                                       currencyFractionName,
                                       Sungero.Core.CaseConverter.ConvertCurrencyNameToTargetDeclension(currencyFractionName, Core.DeclensionCase.Genitive),
                                       Sungero.Core.CaseConverter.ConvertCurrencyNameToTargetDeclension(currencyFractionName.Pluralize(), Core.DeclensionCase.Genitive));
    }
    ...
    /// <returns>Запакованная последовательность идентификаторов.</returns>
    [Public]
    public virtual string PackIds(List<long> ids)
    {
      ...
      var orderedIds = ids.OrderBy(i => i).ToList();
      long startId = orderedIds.First();
      int countIds = orderedIds.Count();
      
      for (int i = 0; i < countIds; i++)
      {
        var id = orderedIds[i];
        var isLastElement = i >= countIds - 1;
        
        if (isLastElement || id + 1 != orderedIds[i + 1])
        {
          if (startId == id)
            result.Add(id.ToString());
          else
            result.Add(string.Format("{0}-{1}", startId, id));
          
          if (!isLastElement)
            startId = orderedIds[i + 1];
        }
      }
    ...
    /// </summary>
    /// <param name="packedIds">Запакованная последовательность идентификаторов.</param>
    /// <returns>Идентификаторы сущностей.</returns>
    [Public]
    public virtual List<long> UnpackIds(string packedIds)
    {
      var ranges = packedIds.Split(',');
      var result = new List<long>();
      
      foreach (var range in ranges)
      {
        var splitedRange = range.Split('-');
        
        if (splitedRange.Count() == 1)
        {
          var id = long.Parse(splitedRange[0]);
          result.Add(id);
        }
        else if (splitedRange.Count() == 2)
        {
          var start = long.Parse(splitedRange[0]);
          var end = long.Parse(splitedRange[1]);
          result.AddRange(this.CreateRange(start, end));
        }
      }
    ...
    /// <summary>
    /// Создать последовательность long чисел.
    /// </summary>
    /// <param name="startValue">Начальное число последовательности.</param>
    /// <param name="endValue">Конечное число последовательности.</param>
    /// <returns>Последовательность long чисел.</returns>
    private IEnumerable<long> CreateRange(long startValue, long endValue)
    {
    ...
    {
      var moduleGuid = Sungero.Docflow.PublicConstants.Module.PowerOfAttorneyKonturGuid;
      if (!Docflow.PublicFunctions.Module.Remote.IsModuleAvailableByLicense(moduleGuid))
      {
        Logger.Debug("IsPoAKonturLicenseEnable. Module license \"PowerOfAttorneyKontur\" not found.");
        return false;
      }
    ...
    /// <summary>
    /// Проверить эл. доверенность/заявление на отзыв перед отправкой на регистрацию в сервисе доверенностей.
    /// </summary>
    /// <param name="document">Документ.</param>
    /// <returns>Сообщение об ошибке или пустая строка, если ошибок нет.</returns>
    [Public]
    public virtual string ValidatePoaDocumentBeforeRegistration(IOfficialDocument document)
    {
      if (document == null || Docflow.OfficialDocuments.As(document) == null)
        return Sungero.Docflow.Resources.PrimaryDocumentNotFoundError;
      
      if (!Sungero.PowerOfAttorneyCore.PublicFunctions.Module.HasPowerOfAttorneyServiceConnection(document.BusinessUnit))
        return Sungero.PowerOfAttorneyCore.Resources.ServiceConnectionNotConfigured;
      
      if (FormalizedPowerOfAttorneys.Is(document))
      {
        // Валидация эл. доверенности.
        var fpoa = FormalizedPowerOfAttorneys.As(document);
        var validationError = Sungero.Docflow.PublicFunctions.FormalizedPowerOfAttorney.Remote.ValidateFormalizedPoABeforeSending(fpoa);
        if (!string.IsNullOrEmpty(validationError))
          return validationError;
        
        if (fpoa.FtsListState == Docflow.FormalizedPowerOfAttorney.FtsListState.Registered)
        {
      ...
      else if (PowerOfAttorneyRevocations.Is(document))
      {
        // Валидация заявления на отзыв эл. доверенности.
        var revocation = PowerOfAttorneyRevocations.As(document);
        var validationError = Sungero.Docflow.PublicFunctions.PowerOfAttorneyRevocation.Remote.ValidateRevocationBeforeSending(revocation);
        if (!string.IsNullOrEmpty(validationError))
          return validationError;
        
        if (revocation.FtsListState == Docflow.PowerOfAttorneyRevocation.FtsListState.Registered)
        {
      ...
      {
        return Sungero.Docflow.Resources.InvalidDocumentKindForFtsRegistration;
      }
    }
    
    /// <summary>
    /// Отправить эл. доверенность/заявление на отзыв регистрацию в сервис доверенностей.
    /// </summary>
    /// <param name="mainDocument">Документ.</param>
    /// <param name="approvalTaskId">ИД задачи на согласование.</param>
    /// <returns>Сообщение об ошибке или пустая строка, если ошибок нет.</returns>
    [Public]
    public virtual string SendPoaDocumentForRegistrationToFts(IOfficialDocument mainDocument, long? approvalTaskId)
    {
      if (FormalizedPowerOfAttorneys.Is(mainDocument))
      {
        var formalizedPoa = FormalizedPowerOfAttorneys.As(mainDocument);
        var sendingResult = Sungero.Docflow.PublicFunctions.FormalizedPowerOfAttorney.Remote.RegisterFormalizedPowerOfAttorneyWithService(formalizedPoa, approvalTaskId);
      ...
      if (PowerOfAttorneyRevocations.Is(mainDocument))
      {
        ...
        var sendingResult = Sungero.Docflow.PublicFunctions.PowerOfAttorneyRevocation.Remote.RegisterRevocationWithService(poaRevocation, approvalTaskId);
        if (!string.IsNullOrEmpty(sendingResult.ErrorCode))
    ...
    /// <summary>
    /// Проверить, что процесс регистрации документа в сервисе доверенностей завершен.
    /// </summary>
    /// <param name="document">Документ.</param>
    /// <returns>True, если процесс регистрации завершен, иначе - false.</returns>
    [Public]
    public virtual bool CheckPoaDocumentRegistrationIsComplete(IOfficialDocument document)
    {
      if (FormalizedPowerOfAttorneys.Is(document))
        return Docflow.PublicFunctions.FormalizedPowerOfAttorney.CheckRegistrationProcessIsComplete(FormalizedPowerOfAttorneys.As(document));
      
      if (PowerOfAttorneyRevocations.Is(document))
        return Docflow.PublicFunctions.PowerOfAttorneyRevocation.CheckRegistrationProcessIsComplete(PowerOfAttorneyRevocations.As(document));
      
      return false;
    }
    
    /// <summary>
    /// Получить текст ошибки регистрации документа в сервисе доверенностей.
    /// </summary>
    /// <param name="document">Документ.</param>
    /// <returns>Сообщение об ошибке регистрации, несоответствии типа документа или пустая строка, если ошибок нет.</returns>
    [Public]
    public virtual string GetPoaDocumentFtsRegistrationError(IOfficialDocument document)
    {
      if (FormalizedPowerOfAttorneys.Is(document))
        return FormalizedPowerOfAttorneys.As(document).FtsRejectReason;
      
      if (PowerOfAttorneyRevocations.Is(document))
        return PowerOfAttorneyRevocations.As(document).FtsRejectReason;
