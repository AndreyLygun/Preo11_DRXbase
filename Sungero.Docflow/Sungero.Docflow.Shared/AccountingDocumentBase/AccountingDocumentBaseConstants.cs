...
namespace Sungero.Docflow.Constants
{
  ...
  {
    // Признак смены типа документа из сервиса обмена с "Вх. док. эл. обмена"
    // на "Вх. счет", "Исх. счет", "Акт выполненных работ" или "Накладная".
    public const string IsChangeTypeExchangeDocument = "isChangeTypeExchangeDocument";
    ...
    // Максимальная длина доп. сведений для подписанта.
    [Sungero.Core.Public]
    public const int SignerAdditionalInfoMaxLength = 255;
    ...
    public static class GenerateTitleTypes
    {
      ...
      public const string SignerAdditionalInfo = "SignerAdditionalInfo";
    }
  }
}