  ...
  partial class EntityReworkAssignmentFunctions
  {
    ...
    public virtual void FillEntityParamsIfEmpty()
    {
      var anyParameterIsMissing =
        ...
        !CommonsPublicFuncs.Module.EntityParamsContainsKey(_obj, Constants.EntityReworkAssignment.SpecifyDeliveryMethodParamName) ||
      ...
      if (anyParameterIsMissing || this.NeedFillMissingReworkBlockParams())
    ...
    /// </summary>
    /// <returns>True - нужно заполнять. False - иначе.</returns>
    public virtual bool NeedFillMissingReworkBlockParams()
    {
      return !Sungero.Commons.PublicFunctions.Module.EntityParamsContainsKey(_obj, Constants.EntityReworkAssignment.CanChangeApprovers) ||
        !Sungero.Commons.PublicFunctions.Module.EntityParamsContainsKey(_obj,  Constants.EntityReworkAssignment.CanChangeApprovalDeadline);
    }
    ...
    {
      return CommonsPublicFuncs.Module
        .GetBooleanEntityParamsValue(_obj, Constants.EntityReworkAssignment.SpecifyDeliveryMethodParamName);
    }
    ...
    public virtual void UpdateStateForExchangeDeliveryMethod(IOfficialDocument officialDocument)
    {
      ...
      var isFormalizedDocument = Docflow.AccountingDocumentBases.As(officialDocument)?.IsFormalized == true;
    ...
    /// </summary>
    /// <remarks>Значение берется из кэша.</remarks>
    /// <returns>True - можно, False - иначе.</returns>
    public virtual bool IsChangeApproversEnabled()
    {
      return Sungero.Commons.PublicFunctions.Module
        .GetBooleanEntityParamsValue(_obj, DocflowApproval.Constants.EntityReworkAssignment.CanChangeApprovers);
    }
    ...
    {
      return Sungero.Commons.PublicFunctions.Module
        .GetBooleanEntityParamsValue(_obj, DocflowApproval.Constants.EntityReworkAssignment.CanChangeApprovalDeadline);
    }
  }
}