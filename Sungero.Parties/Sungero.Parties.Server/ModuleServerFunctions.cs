  ...
  public class ModuleFunctions
  {
    ...
    public static List<Structures.Module.CounterpartyFromExchangeService> FindCompanyInExchangeServices(string tin, string trrc, List<ExchangeCore.IBusinessUnitBox> boxes)
    {
      ...
      foreach (var counterparty in counterparties)
      {
        ...
        foundCompany.Box = ExchangeCore.BusinessUnitBoxes.GetAll().FirstOrDefault(b => b.Id == Convert.ToInt64(items[3]));
    ...
    {
      return Counterparties.GetAll().Where(c => c.ExchangeBoxes.Any(b => Equals(b.Box, box) && b.OrganizationId == organizationId &&
                                                                    (b.CounterpartyBranchId == null || b.CounterpartyBranchId == string.Empty))).FirstOrDefault();
    }
    ...
    /// <param name="trrc">КПП контрагента.</param>
    /// <param name="name">Наименование контрагента.</param>
    /// <returns>Список ИД найденных контрагентов.</returns>
    [Public(WebApiRequestType = RequestType.Get)]
    public virtual List<long> GetCounterpartyIds(string tin, string trrc, string name)
    {
      var counterparties = GetDuplicateCounterparties(tin, trrc, name, false);
      
      return counterparties.Select(c => c.Id).ToList();
    }
    ...
    /// <param name="trrc">КПП организации.</param>
    /// <param name="name">Наименование организации.</param>
    /// <returns>Список ИД найденных организаций.</returns>
    [Public(WebApiRequestType = RequestType.Get)]
    public virtual List<long> GetCompanyIds(string tin, string trrc, string name)
    {
      var counterparties = GetDuplicateCounterparties(tin, trrc, name, false);
      
      return counterparties.Where(c => Companies.Is(c)).Select(c => c.Id).ToList();
    }
    
    /// <summary>
    /// Получить персоны по ИНН и наименованию.
    /// </summary>
    /// <param name="tin">ИНН персоны.</param>
    /// <param name="name">Наименование персоны.</param>
    /// <returns>Список ИД найденных персон.</returns>
    [Public(WebApiRequestType = RequestType.Get)]
    public virtual List<long> GetPersonIds(string tin, string name)
    {
      var counterparties = GetDuplicateCounterparties(tin, null, name, false);
      
      return counterparties.Where(c => People.Is(c)).Select(c => c.Id).ToList();
    }    
    ...
    {
      return Parties.Functions.Bank.GetBankIdsServer();
    }
    ...
    {
      Logger.Debug("ElasticsearchReindex. ReindexCompanyBases. Start.");
      
      Logger.Debug("ElasticsearchReindex. ReindexCompanyBases. Recreate index...");
      ...
      while (true)
      {
        ...
        Logger.DebugFormat("ElasticsearchReindex. ReindexCompanyBases. Indexing companies, id from {0} to {1}...",
                           companies.First().Id, lastId);
      ...
      Logger.Debug("ElasticsearchReindex. ReindexCompanyBases. Finish.");
    }
    ...
    {
      Logger.Debug("ElasticsearchReindex. ReindexContacts. Start.");
      
      Logger.Debug("ElasticsearchReindex. ReindexContacts. Recreate index...");
      ...
      while (true)
      {
        ...
        Logger.DebugFormat("ElasticsearchReindex. ReindexContacts. Indexing contacts, id from {0} to {1}...",
                           contacts.First().Id, lastId);
      ...
      Logger.Debug("ElasticsearchReindex. ReindexContacts. Finish.");
    }
    ...
    /// <summary>
    /// Получить синонимы из индекса компаний.
    /// </summary>
    /// <returns>Строка с синонимами.</returns>
    [Public(WebApiRequestType = RequestType.Get)]
    public virtual string GetCompanyIndexSynonyms()
    {
      var synonyms = string.Empty;
      if (Commons.PublicFunctions.Module.IsElasticsearchEnabled())
      {
        var companyIndexName = Commons.PublicFunctions.Module.GetIndexName(CompanyBases.Info.Name);
        synonyms = string.Join(";" + Environment.NewLine, Commons.PublicFunctions.Module.ElasticsearchGetIndexSynonyms(companyIndexName));
      }
      return synonyms;
    }
    ...
    #region Удостоверения личности
    
    /// <summary>
    /// Проверка, может ли текущий сотрудник редактировать данные документов, удостоверяющих личность.
    /// </summary>
    /// <returns>True - если пользователю разрешено редактировать данные документов, удостоверяющих личность.</returns>
    [Remote(IsPure = true)]
    public virtual bool CanEditIdentityDocuments()
    {
      if (Users.Current.IncludedIn(Docflow.PublicConstants.Module.RoleGuid.IdentificationDocumentsManagersRole))
        return true;
      
      if (Substitutions.SubstitutedUsersWithoutSystem.Any(u => u.IncludedIn(Docflow.PublicConstants.Module.RoleGuid.IdentificationDocumentsManagersRole)))
        return true;
