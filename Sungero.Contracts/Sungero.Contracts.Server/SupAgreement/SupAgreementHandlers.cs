  ...
  partial class SupAgreementLeadingDocumentPropertyFilteringServerHandler<T>
  {
    ...
    public override IQueryable<T> LeadingDocumentFiltering(IQueryable<T> query, Sungero.Domain.PropertyFilteringEventArgs e)
    {
      ...
      query = query.Where(c => !Equals(c.LifeCycleState, Sungero.Contracts.ContractBase.LifeCycleState.Obsolete) &&
                          ...
                          !Equals(c.LifeCycleState, Sungero.Contracts.SupAgreement.LifeCycleState.Obsolete) &&
                          !Equals(c.LifeCycleState, Sungero.Contracts.SupAgreement.LifeCycleState.Closed) &&
                          !Equals(c.LifeCycleState, Sungero.Contracts.SupAgreement.LifeCycleState.Terminated));
      ...
      // Также нельзя выбрать доп. соглашение в качестве ведущего документа для самого себя.
  ...
  partial class SupAgreementServerHandlers
  {
    ...
    public override void BeforeSave(Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      if (_obj.LeadingDocument != null && _obj.LeadingDocument.AccessRights.CanRead() &&
          !_obj.Relations.GetRelatedFromDocuments(Constants.Module.SupAgreementRelationName).Any(x => x.Id == _obj.LeadingDocument.Id))
