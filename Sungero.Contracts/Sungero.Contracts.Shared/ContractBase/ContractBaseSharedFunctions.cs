  ...
  partial class ContractBaseFunctions
  {
    ...
    public override void ChangeDocumentPropertiesAccess(bool isEnabled, bool isRepeatRegister)
    {
      ...
      else
      {
        ...
        var categoryCodeIncludedInNumber = isRepeatRegister && documentRegister != null &&
          Docflow.PublicFunctions.DocumentRegister.NumberFormatContains(documentRegister, FormatElement.CategoryCode);
        
        // Категория договора недоступна при перерегистрации, если в формате номера журнала есть код категории.
        contractProperties.DocumentGroup.IsEnabled = isEnabled && !categoryCodeIncludedInNumber;
        // Признак "Типовой".
        contractProperties.IsStandard.IsEnabled = isEnabled && enabledState;
        // Признак "Рамочный".
        contractProperties.IsFrameworkContract.IsEnabled = isEnabled && enabledState;
      }
    ...
    [Public]
    public override string GetCategoryCode()
    {
      if (_obj.DocumentGroup != null && ContractCategories.Is(_obj.DocumentGroup))
        return ContractCategories.As(_obj.DocumentGroup).Code;
      
      return base.GetCategoryCode();
    }
